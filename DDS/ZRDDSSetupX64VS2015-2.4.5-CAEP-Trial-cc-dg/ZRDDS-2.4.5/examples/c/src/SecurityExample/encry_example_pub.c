...
int pub(int argc, char* argv[])
{
    ...
    // 初始化使用rio的域参与者
    DDS_DomainParticipant* udpDp = DDS_CreateDP(150, "encry_example_pub");
    if (NULL == udpDp) { printf("DDSIF::CreateDP failed.\n"); return -2; }
    // 创建数据写者
    DDS_DataWriter* rawWriter1 = DDS_PubTopic(
        udpDp, "encry_example_1", &DDS_BytesTypeSupport_instance, "reliable", NULL);
    ...
    DDS_DataWriter* rawWriter2 = DDS_PubTopic(
        udpDp, "encry_example_2", &DDS_BytesTypeSupport_instance, "reliable", NULL);
