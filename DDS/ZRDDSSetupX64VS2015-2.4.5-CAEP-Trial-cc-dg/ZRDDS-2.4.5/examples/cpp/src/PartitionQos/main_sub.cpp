...
class tDataReaderListener : public DataReaderListener
{
    void on_data_available(DataReader *reader)
    {
        ...
        for (unsigned int i = 0; i < infoSeq.length(); ++i)
        {
            ...
            else if (dataSeq[i].x == 1)
            {
                printf("group2数据：\n");
            }
            else if (dataSeq[i].x == 2)
            {
                printf("group3数据：\n");
            }
            else
...
int main(int argc, char** argv)
{
    ...
    // 创建数据读者
    DataReader* dr = sub->create_datareader(tp, DATAREADER_QOS_DEFAULT, &tlistener, DATA_AVAILABLE_STATUS);
    if (dr == NULL)
    {
