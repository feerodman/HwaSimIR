...
$.widget( "xml.xmlEditor", {
    options: {
        ...
        documentTitle : null,
        addTopMenuHeaderText : '添加根节点',//'Add Top Element',
        addAttrMenuHeaderText : '添加属性', //'Add Attribute',
        addElementMenuHeaderText : '添加子元素', //'Add Subelement',
        xmlEditorLabel : 'XML',
    ...
    _create: function() {
        ...
        if (typeof(this.options.schema) != 'function') {
            ...
            if (libPath) {
                ...
                this.libPath = this.baseUrl + "static/js/";
            }
    ...
    _init: function() {
        if (this.options.submitButtonConfigs) {
            ...
            this.submitButtonConfigs = [{
                ...
                label : exporting? "导出" : "Submit changes",
    ...
    exportXML: function() {
        ...
        var exportDialog = $("<form><input type='text' class='xml_export_filename' placeholder='ZRDDS_QOS_PROFILES.xml'/><input type='submit' value='导出'/></form>")
                .dialog({modal: true, dialogClass: 'xml_dialog', resizable : false, title: '输入文件名', height: 100});
        var self = this;
        exportDialog.submit(function(){
            ...
            if (!fileName)
                fileName = "ZRDDS_QOS_PROFILES.xml";
            var download = $('<a>下载 ' + fileName + '</a>').attr("href", url);
...
AbstractXMLObject.prototype.createElementInput = function (inputID, startingValue, appendTarget){
    ...
    var input = null;
    var $input = null;
    var needSelect = this.objectType.values && this.objectType.values.length > 0;
    if (!needSelect) {
        if (this.parentElement != null && this.parentElement.objectType.values && this.parentElement.objectType.values.length > 0) {
            needSelect = true;
        }
    }
    ...
    if (needSelect) {
        ...
        if (selectionValues == null) {
            selectionValues = this.parentElement.objectType.values;
        }
...
DocumentState.prototype.updateStateMessage = function () {
    if (this.isChanged()) {
        $("." + submissionStatusClass).html("有未保存修改");
    } else {
        $("." + submissionStatusClass).html("所有修改已保存");
    }
};
...
DocumentState.prototype.addNamespace = function(prefixOrType, namespace) {
    ...
    if (documentElement.setAttributeNS) {
        ...
        if (isIE) {
            documentElement.setAttribute('xmlns-' + nsPrefix, namespace);
        }
        else {
            documentElement.setAttribute('xmlns:' + nsPrefix, namespace);
        }
...
GUIEditor.prototype.selectNode = function(selected) {
    if (!selected || selected.length == 0) {
        ...
        var selectedObject;
        selectedStr = selected.toString();
        if (selectedStr.indexOf("Object]") == -1 || selected instanceof jQuery) {
...
MenuBar.prototype.generateMenuItem = function(menuItemData, parentMenu) {
    ...
    var menuItemLink = $("<a/>").appendTo(menuItem).html("<span>" + menuItemData.displaylabel + "</span>");
...
ModifyMenuPanel.prototype.initialize = function (parentContainer) {
    ...
    if (this.editor.options.enableDocumentStatusPanel) {
        ...
        $("<span/>").addClass(submissionStatusClass).html("文档未修改")
...
function SchemaTree(rootElement) {
    ...
    for (var index = 0; index < this.namespaceIndexes.length; ++index) {
...
SchemaTree.prototype.getElementDefinition = function(elementNode) {
    ...
    for (index = 0; index < defList.length; ++index) {
...
SchemaTree.prototype.pathMatches = function(elementNode, definition) {
    ...
    for (index = 0; index < definition.parents.length; ++index) {
...
XMLTextNode.prototype.addXmlNode = function(prepend) {
    var textValue = "";
    if (!this.textNode) {
        var xmlDocument = this.parentElement.editor.xmlState.xml[0];
        this.textNode = xmlDocument.createTextNode("");
        if (prepend) {
