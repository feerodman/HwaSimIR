...
int sub()
{
    ...
    // 初始化授权的域参与者
    DDS_DomainParticipant* authDP = DDS_CreateDP(150, "encry_example_sub");
    if (NULL == authDP) { printf("DDS_CreateDP failed.\n"); return -2; }
    // 创建encry_example_1数据读者，并设置监听器
    ...
    DDS_DataReader* authDr = DDS_SubTopic(
        authDP, "encry_example_1", &DDS_BytesTypeSupport_instance, "reliable", &m_listener);
    if (NULL == authDr) { printf("DDS_SubscribeTopic failed.\n"); return -5; }
    // 创建encry_example_2数据读者，并设置监听器
    authDr = DDS_SubTopic(
        authDP, "encry_example_2", &DDS_BytesTypeSupport_instance, "reliable", &m_listener);
    if (NULL == authDr) { printf("DDS_SubscribeTopic failed.\n"); return -5; }
