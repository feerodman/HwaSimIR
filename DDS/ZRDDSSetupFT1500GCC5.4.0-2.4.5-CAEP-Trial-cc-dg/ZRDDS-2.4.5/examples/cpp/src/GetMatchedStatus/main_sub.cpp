...
int main(int argc, char** argv)
{
    ...
    {
        //获取所有匹配的wirter句柄
        InstanceHandleSeq hadnleSeq;
        dr->get_matched_publications(hadnleSeq);
        printf("当前匹配%d个数据写者\n", hadnleSeq.length());

        //获取句柄对应的详细信息
        for (int i = 0; i < hadnleSeq.length();i++)
        {
            PublicationBuiltinTopicData publicationData;
            dr->get_matched_publication_data(publicationData, hadnleSeq[i]);
