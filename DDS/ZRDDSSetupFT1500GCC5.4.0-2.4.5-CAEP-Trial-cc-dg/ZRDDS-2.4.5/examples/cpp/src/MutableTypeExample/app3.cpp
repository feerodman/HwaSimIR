...
#include "MutableTypeExampleDataWriter.h"
...
int main()
{
    ...
    // 创建主题
    Topic* topic = dp->create_topic("mutable_example_topic", registeredTypeName, TOPIC_QOS_DEFAULT, NULL, STATUS_MASK_NONE);
    if (NULL == topic) { printf("DomainParticipant::create_topic failed.\n"); return -4; }
    // 创建使用“新”的数据类型的数据写者
    DataWriter* rawWriter = DDSIF::PubTopic(
        dp, "mutable_example_topic", NewTypeTypeSupport::get_instance(), "default", NULL);
