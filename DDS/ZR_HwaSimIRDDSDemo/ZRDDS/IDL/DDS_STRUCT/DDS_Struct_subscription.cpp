...
class  Mylistener : public DDS::DataReaderListener
{
    ...
    void on_data_available(DDS::DataReader *the_reader)
    {
        ...
        // 转化为类型安全的数据读者接口
        ProxyToServDataReader *exampleReader = dynamic_cast<ProxyToServDataReader*> (the_reader);
        ProxyToServSeq dataValues;
        ...
        for (unsigned int i = 0; i < sampleInfos.length(); i++)
        {
            ...
            // TODO 在此处添加对数据的处理逻辑
            ProxyToServPrintData(&dataValues[i]);
        }
...
int main()
{
    ...
    DDS::DataReader *exampleReader = participant->create_datareader_with_topic_and_qos_profile(
        "example",
        ProxyToServTypeSupport::get_instance(),
        "default_lib",
        "default_profile",
