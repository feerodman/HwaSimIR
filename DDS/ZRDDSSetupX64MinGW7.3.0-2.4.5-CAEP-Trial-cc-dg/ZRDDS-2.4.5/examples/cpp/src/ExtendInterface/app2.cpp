...
typedef struct ZRDDSDemoModules
{
    ...
    /** @brief   用于接收主题1数据。 */
    ZeroCopyBytesDataReader* m_airTrackDr;
    /** @brief   数据读者监听器。 */
...
int ZRDDSDemoModulesInitial()
{
    ...
    DataReader* rawReader = g_zrddsModule.m_udpDp->create_datareader_with_topic_and_qos_profile(
        "AirTrack", ZeroCopyBytesTypeSupport::get_instance(), "lib1", "profile1", "AirTrack", g_zrddsModule.m_listener, STATUS_MASK_ALL);
    g_zrddsModule.m_airTrackDr = dynamic_cast<ZeroCopyBytesDataReader*>(rawReader);
