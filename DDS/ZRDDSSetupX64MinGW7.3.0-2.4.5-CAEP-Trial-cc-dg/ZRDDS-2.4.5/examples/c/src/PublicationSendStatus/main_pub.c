...
int main(int argc, char** argv)
{
    ...
    data.x = 0;
    data.y = 0;
    strcpy(data.z, "Data Receive by Listener");

    // 发送状态列表
    DDS_PublicationSendStatusSeq status;
    DDS_PublicationSendStatusSeq_initialize(&status);
    // 发送前获取状态，并打印
    ShapeTypeDataWriter_get_send_status(dw, &status);
    ShapeTypeDataWriter_print_send_status(dw, &status);
