...
$checks.Add((Add-Check "rear hotspot driven by engineState model" (($appSourceText -match "updateEngineRear\(platformName,\s*runtimeKey,\s*engineState") -and ($appSourceText -match '(set_shader_input|SetShaderInputCached)\([^;\r\n]*"u_hotspot_rear_en",\s*LVecBase2i\(rearEnabledForShader')) $appSource)) | Out-Null
