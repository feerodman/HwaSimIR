...
$logDir = Join-Path $Root "logs\phase_h3_headless_perf_ab-$stamp"
...
$cases = @(
    @{ Name = "A_dual_pass_overlay_on_jpeg_rgb_q100"; FastDirect = "false"; Overlay = "true";  JpegMode = "rgb";  JpegQuality = "100" },
    @{ Name = "B_dual_pass_overlay_off_jpeg_rgb_q100"; FastDirect = "false"; Overlay = "false"; JpegMode = "rgb";  JpegQuality = "100" },
    @{ Name = "C_direct_final_overlay_off_jpeg_rgb_q100"; FastDirect = "true"; Overlay = "false"; JpegMode = "rgb";  JpegQuality = "100" },
    @{ Name = "D_direct_final_overlay_off_jpeg_gray_q95"; FastDirect = "true"; Overlay = "false"; JpegMode = "gray"; JpegQuality = "95" }
)
...
try {
    ...
    foreach ($case in $cases) {
        ...
        $runtimeText = Set-IniValueText $runtimeText "HeadlessFastDirectFinal" $case.FastDirect
        ...
        $runtimeText = Set-IniValueText $runtimeText "OverlayInSensorImage" $case.Overlay
        $runtimeText = Set-IniValueText $runtimeText "JsonPerFrame" "true"
        $runtimeText = Set-IniValueText $runtimeText "JpegEncodeMode" $case.JpegMode
        $runtimeText = Set-IniValueText $runtimeText "JpegQuality" $case.JpegQuality
		...
		if (Test-Path -LiteralPath $hwaLog) {
            ...
            $probeLine = Get-Content -LiteralPath $hwaLog | Where-Object { $_.StartsWith("[RenderPerfProbe]") } | Select-Object -Last 1
            $pipelineLine = Get-Content -LiteralPath $hwaLog | Where-Object { $_.StartsWith("[Stage6 FinalPipeline]") } | Select-Object -Last 1
        }

        $summary += [pscustomobject]@{
            Case = $case.Name
            RenderPath = Get-Metric $pipelineLine "renderPath"
            OutputFps = Get-Metric $perfLine "outputFps"
            RenderFps = Get-Metric $perfLine "renderFps"
            RenderMs = Get-Metric $perfLine "renderMs"
            PandaDoFrameMs = Get-Metric $probeLine "pandaDoFrameMs"
