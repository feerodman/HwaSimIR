param(
    [string]$CandidatePath = "logs\p11\modtran\humidity_grid\formal_humidity_rows.csv",
    [string]$FormalPath = "HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\processed\band_lut_si.csv",
    [string]$OutputDirectory = "logs\p11\modtran\humidity_grid\query_check"
)
...
$exe = Join-Path $out "p11_modtran_humidity_query.exe"
$log = Join-Path $out "p11_modtran_humidity_query.log"
& $compiler -std=c++11 -O2 -Wall -Wextra -pedantic `
    -I (Join-Path $root "HwaSim_IR\HwaSim_IR\IR") `
    (Join-Path $root "tools\p11_modtran_humidity_query.cpp") `
    (Join-Path $root "HwaSim_IR\HwaSim_IR\IR\IRModtranRadianceLut.cpp") `
    -o $exe
if ($LASTEXITCODE -ne 0) { throw "Humidity query probe compilation failed" }
$output = & $exe $candidate $formal | Out-String
$exitCode = $LASTEXITCODE
[IO.File]::WriteAllText($log, $output, [Text.UTF8Encoding]::new($false))
Write-Output $output.TrimEnd()
if ($exitCode -ne 0) { throw "Humidity query check failed: $log" }
