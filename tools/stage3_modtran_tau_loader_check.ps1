...
$rootPath = $root.Path
$bandLutPath = Join-Path $rootPath "ConsoleApplication1_LLA\Bin\Config\Atmosphere\MODTRAN\processed\band_lut.csv"
$legacyTransmittancePath = Join-Path $rootPath "transmittance\transmittance_0.3_15.txt"
$loaderHeader = Join-Path $rootPath "ConsoleApplication1_LLA\ConsoleApplication1\IR\IRModtranTauLut.h"
$loaderSource = Join-Path $rootPath "ConsoleApplication1_LLA\ConsoleApplication1\IR\IRModtranTauLut.cpp"
$atmosphereHeader = Join-Path $rootPath "ConsoleApplication1_LLA\ConsoleApplication1\IRSimulation.h"
...
$checks.Add((Add-Check "EnableModtranTauDebug default off" ((Test-TextContains $atmosphereSource "m_modtranTauDebugEnabled\(false\)") -and (Test-TextContains $appSource "EnableModtranTauDebug") -and (Test-TextContains $appSource 'ReadProcessEnvFlag\("EnableModtranTauDebug", false\)')) "$atmosphereSource; $appSource")) | Out-Null
$checks.Add((Add-Check "UseModtranTauForAtmosphere default off" ((Test-TextContains $atmosphereSource "m_useModtranTauForAtmosphere\(false\)") -and (Test-TextContains $appSource "UseModtranTauForAtmosphere") -and (Test-TextContains $appSource 'ReadProcessEnvFlag\("UseModtranTauForAtmosphere", false\)')) "$atmosphereSource; $appSource")) | Out-Null
