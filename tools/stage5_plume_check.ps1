...
$checks.Add((Add-Check "P11 plume exports SI source/emitted radiance and separate emissivity/opacity" (($modelHeaderText -match "coreSourceRadianceWm2SrUm") -and ($modelHeaderText -match "haloSourceRadianceWm2SrUm") -and ($modelHeaderText -match "coreEmittedRadianceWm2SrUm") -and ($modelHeaderText -match "haloEmittedRadianceWm2SrUm") -and ($modelHeaderText -match "coreEffectiveEmissivity") -and ($modelSourceText -match "bandAveragePlanckRadianceWm2SrUm") -and ($modelSourceText -match "sourceRadiance \* opacityOut")) "$modelHeader; $modelSource")) | Out-Null
$checks.Add((Add-Check "engine plume profile exists and contains core/halo" ((Test-Path -LiteralPath $profileJson -PathType Leaf) -and ($profileText -match '"defaults"') -and ($profileText -match '"platforms"') -and ($profileText -match '"F35"') -and ($profileText -match '"AIM120D"') -and ($profileText -match '"AIM9X"') -and ($profileText -match '"P11-CIVIL-VAN"') -and ($profileText -match '"core"') -and ($profileText -match '"halo"')) $profileJson)) | Out-Null
$checks.Add((Add-Check "profile exposes physical two-layer plume fields" (($profileText -match '"temperatureK"') -and ($profileText -match '"opacity"') -and ($profileText -match '"lengthM"') -and ($profileText -match '"radiusRootM"') -and ($profileText -match '"radiusTailM"') -and ($profileText -match '"axialDecay"') -and ($profileText -match '"radialDecay"') -and ($profileText -match '"noiseScale"') -and ($profileText -match '"noiseStrength"') -and ($profileText -match '"bandEmissivity"') -and ($profileText -notmatch '"bandGain"') -and ($profileText -match '"SWIR"') -and ($profileText -match '"MWIR"')) $profileJson)) | Out-Null
$checks.Add((Add-Check "P11 civil van has explicit parser/profile and does not use missile fallback" (($modelSourceText -match '"P11-CIVIL-VAN"') -and ($modelSourceText -match 'strictCivilEngineGate') -and ($profileText -match '"P11-CIVIL-VAN"[\s\S]*?"temperatureK"\s*:\s*575\.0') -and ($profileText -match '"P11-CIVIL-VAN"[\s\S]*?"lengthM"\s*:\s*0\.55') -and ($profileText -match '"enabledByEngineState"\s*:\s*true')) "$modelSource; $profileJson")) | Out-Null
$checks.Add((Add-Check "P11 controlled samples explicitly disable plume fallback" (($modelSourceText -match '"P11-CONTROLLED-SAMPLES"') -and ($profileText -match '"P11-CONTROLLED-SAMPLES"\s*:\s*\{\s*"enabled"\s*:\s*false') -and ($profileText -match '"P11-CONTROLLED-SAMPLES"[\s\S]*?"core"\s*:\s*\{\s*"enabled"\s*:\s*false')) "$modelSource; $profileJson")) | Out-Null
$checks.Add((Add-Check "HwaSimIRRuntime.ini has Stage5Plume core/halo controls" (($runtimeText -match "\[Stage5Plume\]") -and ($runtimeText -match "EnableEnginePlume=1") -and ($runtimeText -match "EnginePlumeProfilePath=Config/IRPlume/engine_plume_profiles\.json") -and ($runtimeText -match "MaxPlumeNodes=16") -and ($runtimeText -match "UseEngineState=1") -and ($runtimeText -match "UseProceduralNoise=1") -and ($runtimeText -match "ForcePlumeVisible=0") -and ($runtimeText -match "PlumeDisplayGain=") -and ($runtimeText -match "PlumeCoreDisplayGain=") -and ($runtimeText -match "PlumeHaloDisplayGain=") -and ($runtimeText -match "PlumeOpacityScale=") -and ($runtimeText -match "PlumeCoreOpacityScale=") -and ($runtimeText -match "PlumeHaloOpacityScale=")) $runtimeIni)) | Out-Null
...
$checks.Add((Add-Check "plume model remains local and transport/display independent" ($modelSourceText -cnotmatch "path_radiance|sky_radiance|solar_irradiance|AGC|MTF|H264|UDP video|wholeTargetHotspot|whole-target hotspot") $modelSource)) | Out-Null
