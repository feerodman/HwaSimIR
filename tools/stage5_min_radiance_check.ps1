...
$forbiddenRuntimeFields = "sky_radiance|solar_irradiance|skyRadiance|solarIrradiance"
$checks.Add((Add-Check "Stage5 path component is explicit legacy/disabled, no sky/solar runtime" (($radianceText -match "pathRadianceSource") -and ($radianceText -match "legacy_empirical|disabled") -and ($radianceText -notmatch $forbiddenRuntimeFields)) "$radianceHeader; $radianceSource")) | Out-Null
