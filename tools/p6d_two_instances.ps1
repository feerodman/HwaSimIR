...
$out=Join-Path $root "logs/p6d/$Name"
New-Item -ItemType Directory -Force $out | Out-Null
$runner=Join-Path $PSScriptRoot 'p6d_run_case.ps1'
$a=$null;$b=$null
try {
    $a=Start-Process powershell -WindowStyle Hidden -PassThru -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',$runner,
        '-Name',"${Name}_a",'-Windows','-Normal','-ProductionDefaults','-Preset','Runtime','-DiagnosticJson',"$root/tools/p6d_inputs/audit.json",'-SkipRemoteCleanup','-PlatId','2001','-SensorId','2',
    ...
    $b=Start-Process powershell -WindowStyle Hidden -PassThru -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',$runner,
        '-Name',"${Name}_b",'-Windows','-Normal','-ProductionDefaults','-Preset','Runtime','-DiagnosticJson',"$root/tools/p6d_inputs/audit.json",'-SkipRemoteCleanup','-PlatId','2002','-SensorId','2',
