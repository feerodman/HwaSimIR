param(
    [string]$CaseRoot='D:\HwaSimIR\logs\p11\modtran\swir_alt1_grid',
    [string]$PcModBin='F:\Programs\PcModWin5\Bin'
)

$ErrorActionPreference='Stop'
$expectedRoot=[IO.Path]::GetFullPath('D:\HwaSimIR\logs\p11\modtran\swir_alt1_grid')
...
$backupRoot=[IO.Path]::Combine($tempBase,'hwasimir-p11-modtran-alt1-'+[Guid]::NewGuid().ToString('N'))
...
if($cases.Count -ne 42){throw "Expected 42 component cases, got $($cases.Count)"}
...
try {
    foreach($case in $cases){
        ...
        if($case.observer_alt_km -and [double]$case.observer_alt_km -ne 1.0){throw "Wrong observer altitude: $($case.case_id)"}
        if([double]$case.target_alt_km -ne 1.0){throw "Wrong target altitude: $($case.case_id)"}
        ...
        $mapping=@{'tape6'='MODOUT1.txt';'tape7'='MODOUT2.txt';'tape8'='MODOUT3.txt';'tape7.scn'='tape7.scn';'specflux'='spectral_flux.flx'}
        foreach($sourceName in $mapping.Keys){
            ...
            if(Test-Path -LiteralPath $source -PathType Leaf){
                ...
                $runs.Add([pscustomobject]@{
                    case_id=$case.case_id;mode=$case.mode;observer_alt_km=$case.observer_alt_km;target_alt_km=$case.target_alt_km
                    generated_at_utc=[DateTime]::UtcNow.ToString('o');engine_path=$exe;engine_file_version=$engineVersion;engine_sha256=$engineHash
                    output_kind=$sourceName;output_file=[IO.Path]::GetFullPath($destination)
    ...
    [ordered]@{
        checked_at_utc=[DateTime]::UtcNow.ToString('o');executable=$exe;file_version=$engineVersion;sha256=$engineHash
        run_status='SUCCESS_REAL_MODTRAN_OUTPUT';requested_component_runs=$cases.Count
        required_products_fresh=(@($runs|Where-Object {!$_.output_fresh}).Count -eq 0)
        license_evidence="All $($cases.Count) component runs exited zero and produced the required fresh MODOUT/spectral-flux products."
    ...
    foreach($name in $fixedNames){
        ...
        $restore+=[pscustomobject]@{
            fixed_name=$name;before_exists=$before[$name].exists;before_size_bytes=$before[$name].size_bytes
            before_sha256=$before[$name].sha256;before_last_write_time_utc=$before[$name].last_write_time_utc
            after_exists=$exists;after_size_bytes=$(if($exists){(Get-Item -LiteralPath $path).Length}else{0});after_sha256=$afterHash
