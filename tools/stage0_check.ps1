...
$udpLocalPort = Match-Value $hwaNetwork '(?m)^\s*localPort\s*=\s*(\d+)'
$udpRemotePort = Match-Value $hwaNetwork '(?m)^\s*remotePort\s*=\s*(\d+)'
$tcpPort = Match-Value $hwaNetwork '(?m)^\s*serverPort\s*=\s*(\d+)'
$tcpIp = Match-Value $hwaNetwork '(?m)^\s*serverIp\s*=\s*([^\r\n;]+)'
$syncMode = if ($hwa -match 'SetRenderMode\s*\(\s*true\s*,\s*0\s*\)') { "sync UDP-driven" } else { "not confirmed" }

$qtLocalIp = Match-Value $qtNetwork '(?m)^\s*localIp\s*=\s*([^\r\n;]+)'
$qtLocalPort = Match-Value $qtNetwork '(?m)^\s*localPort\s*=\s*(\d+)'
$qtRemoteIp = Match-Value $qtNetwork '(?m)^\s*remoteIp\s*=\s*([^\r\n;]+)'
$qtRemotePort = Match-Value $qtNetwork '(?m)^\s*remotePort\s*=\s*(\d+)'
...
$videoFps = Match-Value $qtActive 'm_videoFpsEdit\s*=\s*new\s+QLineEdit\("(\d+)"\)'

$checks.Add((Add-Check "HwaSimIR UDP port" ($udpLocalPort -eq "8888") "local=$udpLocalPort remote=$udpRemotePort"))
$checks.Add((Add-Check "HwaSimIR TCP video port" ($tcpPort -eq "5555") "server=${tcpIp}:$tcpPort"))
$checks.Add((Add-Check "HwaSimIR render mode" ($syncMode -eq "sync UDP-driven") $syncMode))
$checks.Add((Add-Check "DataDrivenTestQT UDP defaults" (($qtLocalIp -ne "not found") -and ($qtLocalPort -eq "9999") -and ($qtRemoteIp -ne "not found") -and ($qtRemotePort -eq "8888")) "local=${qtLocalIp}:${qtLocalPort} remote=${qtRemoteIp}:${qtRemotePort}"))
$checks.Add((Add-Check "DataDrivenTestQT sensor baseline" (($sensorBand -eq "2") -and ($sensorWidth -eq "800") -and ($sensorHeight -eq "800") -and ($videoFps -eq "60")) "band=$sensorBand size=${sensorWidth}x${sensorHeight} videoFps=$videoFps"))
