...
for folder in sorted(OUT.iterdir()):
    ...
    if not (folder/'received.png').exists():continue
    ...
    cases[folder.name]['cpu_render_call_audit']=entries('CloudRenderCallAudit',log)
    thermal=[]
    for line in read(folder/'thermal.jsonl').splitlines():
        try:thermal.append(json.loads(line))
        except json.JSONDecodeError:pass
    cases[folder.name]['thermal_samples']=thermal
    temperatures=[v for row in thermal for v in row.get('temperaturesC',{}).values()]
    cases[folder.name]['thermal_peak_c']=max(temperatures) if temperatures else None
