param(
    ...
    [string]$TargetType='0x11', [switch]$LegacyNozzleAttachment, [switch]$UiResponsive
)
...
$dir=Join-Path $root "logs\p6\$Name"
...
if($Normal){$overrides.Remove('P5Scene');$overrides.Remove('P6Scene');$overrides.Remove('SensorDisplayPreset')}
if($RawDump){
    $overrides.P6DumpPath=$(if($Windows){"$dir\linear"}else{"/userdata/HwaSimIR/logs/p6_$Name"})
    $overrides.P6DumpSeq="$($Rate*3)"
...
$env:P5DdsVideoSamples="$($Rate*[Math]::Min(10,$Seconds))"
