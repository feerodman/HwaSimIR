param(
    [string]$Name='smoke', [int]$DisplayCase=0, [int]$FreezeAfter=0, [int]$DumpSeq=0, [switch]$ProductionDefaults, [string]$Scene='mixed', [string]$View='oblique',
...
$dir=Join-Path $root "logs\p6c\$Name"
...
if($RawDump){
    $overrides.LinearDiagnosticPath=$(if($Windows){"$dir\linear"}else{"/userdata/HwaSimIR/logs/p6c_$Name"})
...
try {
    ...
    if(-not $Windows){
        ...
        if($RawDump){
            & scp.exe -o StrictHostKeyChecking=yes "root@192.168.1.116:/userdata/HwaSimIR/logs/p6c_$Name.pfm" "$dir\linear.pfm"
            if($Preset -eq 'Auto'){
                & scp.exe -o StrictHostKeyChecking=yes "root@192.168.1.116:/userdata/HwaSimIR/logs/p6c_$($Name)_stats.pfm" "$dir\linear_stats.pfm"
            }
            & scp.exe -o StrictHostKeyChecking=yes "root@192.168.1.116:/userdata/HwaSimIR/logs/p6c_$($Name)_rgb8.png" "$dir\linear_rgb8.png"
        }
    }
