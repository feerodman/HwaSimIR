...
- `HwaSim_IR/HwaSim_IR`：当前 HwaSimIR 主体，Panda3D C++，VS2015/v140，带 CMake/RK3588 迁移雏形。核心入口为 `HwaSimIR.cpp/.h`。
...
- 增加阶段 0 短时启动脚本，用于按正确工作目录启动 `HwaSim_IR.exe` 和 `DataDrivenTestQT.exe`，并输出日志到 `logs/stage0`。
...
- 结果：HwaSimIR 通过 MSBuild `Release|x64` 构建成功，输出 `D:\HwaSimIR\HwaSim_IR\Bin\HwaSim_IR.exe`；DataDrivenTestQT 通过 Qt 5.12.12 MinGW release 构建成功，输出 `D:\HwaSimIR\build-DataDrivenTestQT-codex-mingw73_64-Release\release\DataDrivenTestQT.exe`。
...
- 新增 `IR/IRConfig.h/.cpp`，读取 `HwaSim_IR/Bin/Config/SensorWave/default_*.json`，解析谱范围、宽高、FOV、焦距、探元间距、ADC bit、Display bit、NETD。
...
用户补充：
- 目标模型已迁移到 `HwaSim_IR/Bin/Config/TargetLib/models/` 的分目录资产：`f35`、`f22`、`aim120`、`aim9x`。
...
修改文件：
- `HwaSim_IR/HwaSim_IR/Common/CommonDefine.h`
- `HwaSim_IR/HwaSim_IR/HwaSimIR.h`
- `HwaSim_IR/HwaSim_IR/HwaSimIR.cpp`
- `HwaSim_IR/HwaSim_IR/IRSimulation.h`
- `HwaSim_IR/HwaSim_IR/IRSimulation.cpp`
...
- `HwaSim_IR/Bin/Config/TargetLib/models/aim120/AIM120.mtl`
- `HwaSim_IR/Bin/Config/TargetLib/models/aim9x/aim9x.mtl`
- `HwaSim_IR/Bin/Config/TargetLib/models/f22/f22.mtl`
...
验证：
- 命令：`python tools\modtran\parse_modout2.py --input HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\templates\MWIR_transmittance_MODOUT2.txt --band MWIR --mode Transmittance --output HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\samples\MWIR_transmittance_parse_smoke.csv`
- 结果：通过，解析 1334 行。
- 命令：`python tools\modtran\parse_modout2.py --input HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\templates\MWIR_thermal_MODOUT2.txt --band MWIR --mode ThermalRadiance --output HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\samples\MWIR_thermal_parse_smoke.csv`
- 结果：通过，解析 1334 行。
- 命令：`python tools\modtran\parse_modout2.py --input HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\templates\MWIR_solar_MODOUT2.txt --band MWIR --mode DirectSolarIrradiance --output HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\samples\MWIR_solar_parse_smoke.csv`
- 结果：通过，解析 1341 行。
- 命令：`python tools\modtran\parse_modout2.py --input HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\templates\NIR_scattering_MODOUT2.txt --band NIR --mode RadianceWithScattering --output HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\samples\NIR_scattering_parse_smoke.csv`
- 结果：通过，解析 5196 行。
...
- 每个 case 的 `modin`、`MODOUT1`、`MODOUT2` 已保存到 `HwaSim_IR/Bin/Config/Atmosphere/MODTRAN/raw/samples/<case_id>/`。
...
- 命令：`python tools\modtran\build_validation_band_lut.py --processed-dir HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\processed`
...
验证：
- 命令：`python tools/modtran/check_validation_outputs.py --processed-dir HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\processed`
...
- 命令：`python tools\modtran\build_band_lut.py --processed-dir HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\processed`
...
验证：
- 命令：`python tools\modtran\check_visibility_effect.py --processed-dir HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\processed --raw-dir HwaSim_IR\Bin\Config\Atmosphere\MODTRAN\raw\samples`
...
完成内容：
- 新增 `IR/IRModtranTauLut.h/.cpp`，加载 `HwaSim_IR/Bin/Config/Atmosphere/MODTRAN/processed/band_lut.csv`，只读取 `band`、`atmosphere_model`、`aerosol_model`、`humidity_profile`、`visibility_km`、`observer_alt_km`、`target_alt_km`、`range_km`、`solar_zenith_deg`、`tau_up_band`、`tau_down_band`。
...
修改文件：
- `HwaSim_IR/HwaSim_IR/IR/IRModtranTauLut.h`
- `HwaSim_IR/HwaSim_IR/IR/IRModtranTauLut.cpp`
- `HwaSim_IR/HwaSim_IR/IRSimulation.h`
- `HwaSim_IR/HwaSim_IR/IRSimulation.cpp`
- `HwaSim_IR/HwaSim_IR/HwaSimIR.cpp`
- `HwaSim_IR/HwaSim_IR/CMakeLists.txt`
...
- 更新 `.gitignore`，继续忽略 `HwaSim_IR/Bin/Config/Atmosphere/MODTRAN/generated/`、`raw/samples/`、`raw/failed/`、`raw/archive/`、`processed_snapshots/`，并补充忽略 production 级大型 `*_lut_spectral.csv` 和本地 Panda3D cache；未删除任何数据。
...
- 新增 `HwaSim_IR/Bin/Config/IRHotspots/target_hotspots.json`，为 `F35`、`AIM120`、`AIM120D`、`AIM9X` 写入默认发动机尾部热源和头部/中部 BrightSpot 配置；`F22` 仅作为可选配置，不参与协议生成。
...
完成内容：
- 新增 `HwaSim_IR/Bin/Config/IRRadiance/stage5_debug_display.json`，按 `VIS/NIR/SWIR/MWIR/LWIR` 配置 Stage5 debug 显示参数。
...
- 新增 `HwaSim_IR/Bin/Config/HwaSimIRRuntime.ini`，集中管理 Stage3/4/5/6/7 与 Debug 运行开关，并用中文注释说明参数用途。
...
- 新增 `HwaSim_IR/Bin/Config/Weather/weather_profiles.json`，定义 `envSky=0..5` 对应 Clear、Cloudy、Rain、Snow、Fog、Overcast，并配置 sky/ground 灰度缩放、云量、云温、雾密度、雨雪类型、太阳直射缩放、天空漫射缩放和目标对比度缩放。
- 新增 `HwaSim_IR/Bin/Config/Weather/weather_textures.json`，索引 `cloud_*`、`rain_shaft`、`rain_rgba`、`snow_rgba`、`sun`、`moon` 等贴图；资源复制到 `HwaSim_IR/Bin/Config/Weather/Textures/`。
...
- 新增 `HwaSim_IR/Bin/Config/IRPlume/engine_plume_profiles.json`，包含 `defaults` 与 `F35`、`AIM120D`、`AIM9X` 平台 profile。
