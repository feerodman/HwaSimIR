...
这是均匀可见光场下的单像元参考计算。仅使用人工 400–700 nm 光谱、人工 QE 和人工像元参数；不链接 HwaSimIR、业务目标、热点、跟踪或状态模块，不写入生产配置。原有 `sensor_lab.py` 和 `synthetic_visible.json` 是本轮开始时已有的未提交文件，保持原字节；本轮新增 CSV/严格 JSON 适配、独立入口、参考测试及结果记录。
...
限制：这里没有器件实测 SRF、真实光学系统、温度依赖暗电流、PRNU/DSNU、空间 MTF、滚动快门、色彩标定或真实相机校准。PCG64 可在记录的 NumPy 环境中复现；不声明任意未来 NumPy 版本输出逐字节不变。当前渲染器公共缩放 PFM 无谱辐照度单位，不能接入此实验冒充测量值。
