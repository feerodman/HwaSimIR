...
void TcpCommThread::resetFrameCounters()
{
	{
		std::lock_guard<std::mutex> lock(m_frameMtx);
		// Preserve the no-drop queue while a DDS/recording round is active.
...
bool TcpCommThread::connectToServer()
{
	...
	const int originalFlags = fcntl(tcpSocket, F_GETFL, 0);
	if (originalFlags == -1 || fcntl(tcpSocket, F_SETFL, originalFlags | O_NONBLOCK) == -1)
	{
		close(tcpSocket);
		return false;
	}
	if (connect(tcpSocket, reinterpret_cast<sockaddr*>(&m_serverAddr), sizeof(m_serverAddr)) == -1)
	{
		...
		if (select(tcpSocket + 1, nullptr, &writeSet, nullptr, &timeout) <= 0)
		{
			close(tcpSocket);
			return false;
		}
		...
		if (getsockopt(tcpSocket, SOL_SOCKET, SO_ERROR, &socketError, &socketErrorLength) == -1 || socketError != 0)
		{
			close(tcpSocket);
			return false;
		}
	}
	if (fcntl(tcpSocket, F_SETFL, originalFlags) == -1)
	{
		close(tcpSocket);
		return false;
	}
...
{
    if(!m_workerCpuList.empty()){
        cpu_set_t allowed,chosen;CPU_ZERO(&allowed);CPU_ZERO(&chosen);
        bool valid=sched_getaffinity(0,sizeof(allowed),&allowed)==0;int count=0;
        std::istringstream list(m_workerCpuList);std::string token;
        while(std::getline(list,token,',')){
            char* end=nullptr;const long cpu=std::strtol(token.c_str(),&end,10);
            if(end==token.c_str()||*end||cpu<0||cpu>=CPU_SETSIZE||!CPU_ISSET(cpu,&allowed)){valid=false;break;}
            CPU_SET(cpu,&chosen);++count;
        }
        const int rc=valid&&count?pthread_setaffinity_np(pthread_self(),sizeof(chosen),&chosen):EINVAL;
        std::cout<<"[VideoWorkerPlacement] requested="<<m_workerCpuList<<" result="<<(rc==0?"PASS":"FALLBACK_INHERITED")
            <<" code="<<rc<<" currentCpu="<<sched_getcpu()<<" systemGovernorsChanged=0\n";
    }
	EncodedVideoFrame encodedFrame;
	...
	while (m_bIsRunning)
	{
		...
		std::string h264RequestedCodec = "none", h264RequestedBackend = "none";
		std::string jpegRequestedCodec = "none", jpegRequestedBackend = "none";
		...
		if (needH264)
		{
			...
			if (h264Ok) ++m_h264EncodeCounter;
				...
				std::cerr << "[DdsVideo][FATAL] codec=h264 publishSkipped=1 reason=h264_encode_failed" << std::endl;
		}
		...
		else if (ddsRaw)
		{
			...
			for (int y = 0; y < frame.height; ++y)
			{
				...
				{
					const unsigned int b = src[x * 3 + 0], g = src[x * 3 + 1], r = src[x * 3 + 2];
		...
		std::string requestedCodec = "none", requestedBackend = "none";
		if (includeVideo)
		{
			if (tcpH264 && h264Ok)
			{
		...
		if (productOrdinal <= 3 || (productOrdinal % 120) == 0)
			std::cout << "[VideoOutputProducts] frame=" << productOrdinal
				<< " needH264=" << (needH264 ? 1 : 0) << " h264EncodeCount=" << (h264Ok ? 1 : 0)
				<< " needJpeg=" << (needJpeg ? 1 : 0) << " jpegEncodeCount=" << (jpegOk ? 1 : 0)
				<< " tcpH264=" << (tcpH264 && h264Ok ? 1 : 0)
				<< " ddsH264=" << (ddsH264 && h264Ok ? 1 : 0) << " ddsRaw=" << (ddsRaw ? 1 : 0)
