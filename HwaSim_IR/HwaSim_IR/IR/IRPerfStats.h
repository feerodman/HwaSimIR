...
public:
	...
	void recordDdsRealtimeIngress(std::int64_t callbackSteadyNs);
	void recordAppRealtimeQueued(std::uint64_t sourceSeq, bool ddsIngress, int queueDepth);
	void recordAppRealtimeConsumed(std::uint64_t sourceSeq, bool ddsIngress, int queueDepth);
	void recordDdsRealtimeOverwritten(std::uint64_t count);
	void recordInputBackpressure(double waitMs);
	void recordCatchUpFrame(int burstLength);
	void recordQueueRecovery(double recoveryMs);
	void recordOutputInputUsage(bool uniqueInput);
...
private:
	...
	std::int64_t m_realtimeIngressLastLogNs = 0;
	std::int64_t m_lastDdsCallbackNs = 0;
	std::uint64_t m_ddsRealtimeReceived = 0;
	std::uint64_t m_ddsRealtimeCallbackCount = 0;
	std::uint64_t m_appRealtimeQueued = 0;
	std::uint64_t m_appRealtimeConsumed = 0;
	std::uint64_t m_ddsRealtimeConsumed = 0;
	std::uint64_t m_ddsRealtimeOverwritten = 0;
	std::uint64_t m_ddsRealtimePending = 0;
	std::uint64_t m_firstSourceSeq = 0;
	std::uint64_t m_lastSourceSeq = 0;
	std::uint64_t m_consumedSourceSeq = 0;
	std::uint64_t m_sourceSeqGapCount = 0;
	std::uint64_t m_callbackGapSamples = 0;
	std::uint64_t m_arrivalGapLe1msCount = 0;
	double m_callbackGapUsTotal = 0.0;
	double m_callbackGapUsMax = 0.0;
	std::uint64_t m_currentCallbackBurst = 0;
	std::uint64_t m_burstSize1 = 0;
	std::uint64_t m_burstSize2 = 0;
	std::uint64_t m_burstSize3 = 0;
	std::uint64_t m_burstSize4Plus = 0;
	std::uint64_t m_inputBackpressureCount = 0;
	double m_inputBackpressureWaitMs = 0.0;
	double m_maxInputBackpressureWaitMs = 0.0;
	std::uint64_t m_catchUpFrames = 0;
	std::uint64_t m_maxCatchUpBurst = 0;
	double m_queueRecoveryMs = 0.0;
	std::uint64_t m_uniqueInputFrames = 0;
	std::uint64_t m_repeatedStateOutputFrames = 0;
};
