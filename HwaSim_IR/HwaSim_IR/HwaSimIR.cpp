...
bool FileFnv1a64(const std::string& path, std::uint64_t& hash)
{
...
void WarnIfMaterialCopiesDrift(const std::string& label,
	const std::string& configPath, const std::string& rootPath)
{
	std::uint64_t configHash = 0;
	std::uint64_t rootHash = 0;
	if (!FileFnv1a64(configPath, configHash) || !FileFnv1a64(rootPath, rootHash)) return;
	if (configHash == rootHash) return;
	std::cout << "[MaterialDeployment][WARN] resource=" << label
		<< " configPath=" << AbsolutePathForLog(configPath)
		<< " repoRootPath=" << AbsolutePathForLog(rootPath)
		<< " hashAlgorithm=FNV1a64"
		<< " configHash=" << std::hex << configHash
		<< " repoRootHash=" << rootHash << std::dec
		<< " reason=config_and_repo_root_copies_drifted"
		<< std::endl;
}
...
GraphicsOutput* MakeStage6OffscreenOutput(
	...
	bool floatColor = false)
{
	...
	GraphicsPipe* pipe = host != nullptr ? host->get_pipe() : framework->get_default_pipe();
	GraphicsEngine* engine = framework->get_graphics_engine();
	if (pipe == nullptr || engine == nullptr)
	{
		return nullptr;
	}
	...
	props.set_size(std::max(1, width), std::max(1, height));
...
HwaSimIR::HwaSimIR(int argc, char** argv, const HwaSimIRLaunchOptions& launchOptions)
	...
	m_startupBeginTimeNs = IRPerfStats::steadyTimeNs();
	...
	std::cout << std::fixed << std::setprecision(3)
		<< "[StartupTiming] milestone=framework_open elapsedMs="
		<< (IRPerfStats::steadyTimeNs() - m_startupBeginTimeNs) / 1.0e6
		<< " mode=" << m_renderPresentationModeName << std::endl;
	if (m_msaaSamples > 0)
	{
	...
	// 初始化通讯线程
		...
		std::cout << std::fixed << std::setprecision(3)
			<< "[StartupTiming] milestone=dds_ready elapsedMs="
			<< (IRPerfStats::steadyTimeNs() - m_startupBeginTimeNs) / 1.0e6
			<< std::endl;
		#else
	...
	// 初始化平台模型路径
		...
		std::cout << std::fixed << std::setprecision(3)
			<< "[StartupTiming] milestone=graphics_context_ready elapsedMs="
			<< (IRPerfStats::steadyTimeNs() - m_startupBeginTimeNs) / 1.0e6
			<< " finalPipelineReady=" << (m_stage6FinalPipelineReady ? "1" : "0")
			<< std::endl;
		...
		std::cout << std::fixed << std::setprecision(3)
			<< "[StartupTiming] milestone=resources_ready elapsedMs="
			<< (IRPerfStats::steadyTimeNs() - m_startupBeginTimeNs) / 1.0e6
			<< " finalPipelineReady=" << (m_stage6FinalPipelineReady ? "1" : "0")
			<< std::endl;
...
void HwaSimIR::run() {
	...
	while (true) {
		...
		m_frameRenderImageSeqAfter = m_renderTex != nullptr
			? static_cast<std::uint64_t>(m_renderTex->get_image_modified().get_seq())
			: 0;
		m_frameRenderImageAdvanced = m_frameRenderImageSeqAfter != m_frameRenderImageSeqBefore;
...
void HwaSimIR::resize_window(int new_width, int new_height) {
	if (IsHeadlessOffscreenMode()) {
		...
		if (m_stage6DiagnosticsEnabled) std::cout << "[Stage6 Resize]"
			...
			<< " pipelineReady=" << (m_stage6FinalPipelineReady ? "1" : "0")
			<< std::endl;
		return;
	}
...
void HwaSimIR::ApplyStage6DisplayConfig(const BYHWICD::trackerSensorParam& sensor, const char* reason)
{
    ...
    std::string statsHzSource,statsSizeSource,lowSource,highSource,smoothingSource;
    m_stage6AgcUpdateHz=m_runtimeConfig.getDouble("Stage6AGC","AGCUpdateHz","AGCUpdateHz",preset.statisticsHz,&statsHzSource);
    m_agcSampleSize=m_runtimeConfig.getInt("Stage6AGC","SampleSize","Stage6AGCSampleSize",preset.statisticsSize,&statsSizeSource);
    m_agcSampleSize=std::max(16,std::min(128,m_agcSampleSize));
...
void HwaSimIR::SetupStage6FinalPipeline(int width, int height, const char* reason)
{
	...
	if (IsVisibleWindowMode() && (!m_pMainWindow || !m_pGraphicsWindow || m_cameraNode.is_empty()))
	{
	...
	if (headlessMode && (!m_pFramework || m_cameraNode.is_empty()))
	{
		...
		std::cout << "[Stage6 FinalPipeline][WARN]"
			...
			<< " rawBufferReady=0"
			<< " finalBufferReady=0"
			<< " failure=headless_camera_unavailable"
			<< std::endl;
		return;
	}
	...
	{
		if(m_agcSampleBuffer){m_pFramework->get_graphics_engine()->remove_window(m_agcSampleBuffer);m_agcSampleBuffer=nullptr;
            m_agcSampleRoot.remove_node();m_agcSampleRoot=NodePath();}
	...
	if (IsHeadlessOffscreenMode() && m_stage6FinalSensorBuffer != nullptr)
	{
		GraphicsEngine::get_global_ptr()->remove_window(m_stage6FinalSensorBuffer);
		m_stage6FinalSensorBuffer = nullptr;
		m_stage6FinalRegion = nullptr;
		m_annotationRegion = nullptr;
		m_stage6PresentationOutput = nullptr;
	}
	...
	{
		m_stage6RawSceneRegion->set_camera(m_cameraNode);
	}
	m_stage6RawSceneRegion->set_dimensions(0.0f, 1.0f, 0.0f, 1.0f);
	...
	if (IsHeadlessOffscreenMode())
	{
		...
		if (m_stage6FinalSensorBuffer == nullptr)
		{
			...
			std::cout << "[Stage6 FinalPipeline][WARN]"
				...
				<< " rawSceneTex=Stage6RawSceneTex"
				<< " finalSensorTex=Stage6FinalSensorTex"
				<< " rawBufferReady=" << (m_stage6RawSceneBuffer != nullptr ? "1" : "0")
				<< " finalBufferReady=0"
				<< " tcpSource=final_sensor"
				<< " failure=final_sensor_buffer_unavailable"
				<< std::endl;
	...
	else if (m_stage6FinalRegion == nullptr)
	{
...
void HwaSimIR::SetupAnnotationOverlayRegion(const char* reason)
{
	...
	if (presentationOutput == nullptr)
	{
		std::cout << "[AnnotationOverlay][WARN]"
			<< " renderMode=" << m_renderPresentationModeName
	...
	const char* presentationOutputName = IsHeadlessOffscreenMode() ? "final_sensor_buffer" : "window";
...
void HwaSimIR::UpdateStage6AgcFromFrame(const unsigned char* frameData, int frameWidth, int frameHeight, std::uint64_t sourceSeq)
{
	...
	else
	{
        ...
        const int sampleH=fullReference?m_stage6FinalHeight:m_agcSampleSize;
        if(!ReadSceneLinear(m_pFramework->get_graphics_engine(),fullReference?m_stage6RawSceneTex.p():m_agcSampleTexture.p(),
                fullReference?m_stage6RawSceneBuffer.p():m_agcSampleBuffer.p(),sampleW,sampleH,linear)) {
...
void HwaSimIR::LogStage6ViewportDiag(const char* reason) const
{
	...
	const bool finalBufferReady = headlessMode ? (m_stage6FinalSensorBuffer != nullptr) : (m_pGraphicsWindow != nullptr);
	const char* presentationOutputName = headlessMode ? "final_sensor_buffer" : "window";
	const int renderEffectiveW = headlessMode && m_stage6FinalWidth > 0 ? std::min(renderW, m_stage6FinalWidth) : renderW;
...
void HwaSimIR::ApplyStage7WeatherInputs(NodePath& node, const IRStage7WeatherState& weatherState)
{
	...
	SetShaderInputCached(node, "u_stage7_cloud_optical_depth", LVecBase2f(static_cast<float>(weatherState.cloudOpticalDepth * m_stage7CloudOpticalDepthScale), 0.0f));
	SetShaderInputCached(node, "u_stage7_cloud_mask_channel", LVecBase2i(weatherState.cloudMaskUsesAlpha ? 1 : 0, 0));
...
void HwaSimIR::InitStage7CloudRenderer()
{
    ...
    if(m_cloudSheetWorldRoot.is_empty())m_cloudSheetWorldRoot=m_renderRoot.attach_new_node("PublicCloudWorldSheets");
	...
	{
		NodePath cloud = m_cloudSheetWorldRoot.attach_new_node(
		...
		LMatrix4f cloudInverse;cloudInverse.invert_from(CloudWorldRenderMatrix());
        cloud.set_shader_input("u_cloud_render_to_world",cloudInverse);
...
void HwaSimIR::SetupStage7VolumetricComposite(const char* reason)
{
	...
	// Keep the volumetric pass on the raw-scene output, after the opaque
	// display region.  It then depth-tests against the output's native depth
	// renderbuffer.  This is both cheaper and, importantly, avoids two broken
	// GLES paths on the RK3588 g6p0 driver: replacing the valid renderbuffer
	// with an auto-formatted depth texture makes the FBO incomplete (0x506),
	// while copying the depth plane to a texture produces 0x500/0x502.
	// No CPU readback or second target render is involved.
	m_stage7SceneDepthTex = nullptr;
	...
	if (m_stage7VolumeRegion == nullptr ||
		m_stage7VolumeRegion->get_window() != m_stage6RawSceneBuffer)
	{
		if (m_stage7VolumeRegion != nullptr)
		{
			m_stage7VolumeRegion->set_active(false);
		}
		m_stage7VolumeRegion = m_stage6RawSceneBuffer->make_display_region(0.0f, 1.0f, 0.0f, 1.0f);
	}
	...
	m_stage7VolumeRegion->set_clear_depth_active(false);
	...
	for (size_t i = 0; i < m_stage7CloudVolumePool.size(); ++i)
	{
		if (!m_stage7CloudVolumePool[i].node.is_empty())
		{
			m_stage7CloudVolumePool[i].node.set_shader_input("u_use_scene_depth", LVecBase2i(0, 0));
		}
	}
	std::cout << "[World3DCloudDepthComposite]"
		<< " enabled=1 depthSource=shared_raw_scene_native_depth"
		<< " depthTransfer=none gpuDepthTest=1 cpuDepthReadback=0"
		<< " rawSceneExcludesVolume=1 sort=150"
		<< " reason=" << (reason != nullptr ? reason : "unknown")
		<< std::endl;
}

#include "IR/IRCloudWorldRuntime.inl"
...
void main() {
    ...
    if(u_game_cloud==2){
        ...
        vec3 d=(vec3(1.0,0.56,1.0)-rayOrigin)/safeDirection;
        vec3 lo=min(a,d),hi=max(a,d);
        entryT=max(entryT,max(lo.x,max(lo.y,lo.z)));
        exitT=min(exitT,min(hi.x,min(hi.y,hi.z)));
        if(exitT<=entryT)discard;
    }
    int stepCount = clamp(u_ray_steps, 2, 64);
    ...
    // Smooth prebuilt density + tight bounds use midpoint integration. A
    // screen-periodic phase produced visible diagonal grain in large clouds.
...
void HwaSimIR::InitStage7VolumetricCloudRenderer()
{
	...
	const bool worldGame=!m_p6.enabled&&m_cloudAppearance.enabled;
    const bool gameDensity=(m_p6.enabled&&!m_p6.legacy)||worldGame;
    const int size = worldGame?m_cloudAppearance.size:gameDensity?96:m_stage7VolumeDensityTextureSize;
	...
	for (int templateIndex = 0; templateIndex < m_stage7VolumeDensityTemplateCount; ++templateIndex)
	{
        ...
        if(gameDensity){
            PTA_uchar assetVoxels;
            if(worldGame){
                const auto bytes=m_cloudAppearance.readTemplate(templateIndex);
                assetVoxels=PTA_uchar::empty_array(bytes.size());std::copy(bytes.begin(),bytes.end(),assetVoxels.begin());
                const auto& t=m_cloudAppearance.templates.at(templateIndex);
    ...
    if(m_cloudVolumeWorldRoot.is_empty())m_cloudVolumeWorldRoot=volumeParent.attach_new_node("PublicCloudWorldVolumes");
    else m_cloudVolumeWorldRoot.reparent_to(volumeParent);
    RefreshCloudWorldFrame();
	...
	for (size_t i = 0; i < m_stage7CloudVolumePool.size(); ++i)
	{
		...
		volume.node = m_cloudVolumeWorldRoot.attach_new_node(CreateStage7CloudVolumeProxyNode(static_cast<int>(i)));
...
double HwaSimIR::Stage7CloudLinearValue(IRBand band, double temperatureK, const IRStage7WeatherState& weather) const
{
    ...
    if(!m_p6.enabled&&m_cloudAppearance.enabled)return band==IRBand::NearInfrared?m_cloudAppearance.cloudNir:m_cloudAppearance.cloudMwir;
...
void HwaSimIR::UpdateStage7VolumetricClouds(const IRStage7WeatherState& weatherState, double currentTime, bool stateChanged)
{
	...
	if (weatherAllowsCloud)
	{
		desired = m_stage7VolumeStreaming.queryCandidates(
			streamingCenter[0], streamingCenter[1], m_p6.enabled?m_stage7GroundReferenceZ:0.0,
	...
	for (size_t i = 0; i < desired.size(); ++i)
	{
		...
		for (size_t poolIndex = 0; poolIndex < m_stage7CloudVolumePool.size(); ++poolIndex)
		{
			...
			volume.descriptor = desired[i];
			AuditCloudDescriptor(desired[i]);
			volume.node.set_pos(
	...
	for (size_t i = 0; i < m_stage7CloudVolumePool.size(); ++i)
	{
		...
		const LPoint3f cloudWorld=volume.node.get_pos(m_renderRoot);
	...
	for (size_t order = 0; order < visibleOrder.size(); ++order)
	{
        ...
        if(!m_p6.enabled&&m_cloudAppearance.enabled)
            volume.node.set_shader_input("u_optical_depth",LVecBase2f(float(m_cloudAppearance.opticalDepth*weatherState.cloudOpacity),0));
	}
...
void HwaSimIR::UpdateStage7VolumetricCloudAnimation(double currentTime)
{
	...
	for (size_t i = 0; i < m_stage7CloudVolumePool.size(); ++i)
	{
		...
		volume.node.show();
        if(!m_p6.enabled&&std::getenv("WorldCloudAuditHideId")&&IRWorldCloudStreaming::cloudIdText(volume.descriptor.cloudId)==std::getenv("WorldCloudAuditHideId"))volume.node.hide();
...
int HwaSimIR::RefreshStage7WeatherTextureCache(const IRStage7WeatherState& weatherState)
{
	...
	if (!m_cloudNodes.empty())
	{
		...
		if (resolvedCloudTexturePath != m_stage7CachedCloudTexturePath)
		{
			...
			{
				std::map<std::string, PT(Texture)>::const_iterator cached = m_stage7WeatherTextureCache.find(resolvedCloudTexturePath);
				if (cached != m_stage7WeatherTextureCache.end())
				{
					m_stage7CloudTexture = cached->second;
				}
				else
				{
					m_stage7CloudTexture = TexturePool::load_texture(Filename::from_os_specific(resolvedCloudTexturePath));
					m_stage7WeatherTextureCache[resolvedCloudTexturePath] = m_stage7CloudTexture;
					++loadCount;
					std::cout << "[WeatherTextureLoaded]"
						<< " key=" << weatherState.cloudTextureKey
						<< " path=" << resolvedCloudTexturePath
						<< " success=" << (m_stage7CloudTexture ? "1" : "0")
						<< std::endl;
				}
			}
		}
...
void HwaSimIR::UpdateStage7CloudWorldGrid(const IRStage7WeatherState& weatherState, double currentTime, bool stateChanged)
{
	...
	const LPoint3f cameraPos = CloudRenderToWorld(m_cameraNode.is_empty()
		? LPoint3f(0.0f, 0.0f, 0.0f)
		: m_cameraNode.get_pos(m_renderRoot));
	const double tileSizeM = std::max(100.0, m_stage7CloudTileSizeM);
	...
	const double belowDistance = ((m_p6.enabled?m_stage7GroundReferenceZ:0.0) + m_stage7CloudBaseAltitudeM) - cameraPos[2];
	...
	for (size_t i = 0; i < sliceCount; ++i)
	{
		...
		const double layerT = sliceCount > 1
			? static_cast<double>(i) / static_cast<double>(sliceCount - 1)
			: 0.0;
		const double altitudeM = (m_p6.enabled?m_stage7GroundReferenceZ:0.0) + m_stage7CloudBaseAltitudeM +
			layerT * m_stage7CloudThicknessM;
	...
	if (gridMoved)
	{
		std::cout << "[WeatherCloudGrid]"
			...
			<< " baseZ=" << (m_p6.enabled?m_stage7GroundReferenceZ:0.0) + m_stage7CloudBaseAltitudeM
			<< " topZ=" << cloudTopM
	...
	if (gridMoved || m_stage7CloudSpatialLogCounter <= 3 || (m_stage7CloudSpatialLogCounter % 600) == 0)
	{
		const LPoint3f cloudCenterWorld(
			static_cast<float>(m_stage7CloudGridOriginX),
			static_cast<float>(m_stage7CloudGridOriginY),
			static_cast<float>((m_p6.enabled?m_stage7GroundReferenceZ:0.0) + m_stage7CloudBaseAltitudeM));
		...
		std::cout << "[WeatherCloudSpatial]"
			...
			<< " cloudBaseZ=" << (m_p6.enabled?m_stage7GroundReferenceZ:0.0) + m_stage7CloudBaseAltitudeM
			<< " cloudTopZ=" << cloudTopM
...
{
	if (!m_stage7CloudLayerEnabled)
	{
		return;
	}
	for (size_t i = 0; i < m_cloudNodes.size(); ++i)
	{
		if (!m_cloudNodes[i].is_empty() && !m_cloudNodes[i].is_hidden())
		{
...
void HwaSimIR::LogStage7Weather(const IRStage7WeatherState& weatherState, const char* reason, bool forceLog)
{
	...
	state << weatherState.envSky
		...
		<< ":" << static_cast<int>(weatherState.cloudOpacity * 1000.0)
		<< ":" << static_cast<int>(weatherState.cloudTemperatureK * 10.0)
		<< ":" << static_cast<int>(weatherState.cloudOpticalDepth * 1000.0)
		<< ":" << weatherState.cloudTextureKey
		<< ":" << IRBandName(BuildRuntimeEnvironment().band)
	...
	std::cout << "[WeatherCloud]"
		...
		<< " cloudTempK=" << weatherState.cloudTemperatureK
		<< " band=" << IRBandName(BuildRuntimeEnvironment().band)
		<< " opticalDepth=" << weatherState.cloudOpticalDepth * m_stage7CloudOpticalDepthScale
	...
	{
		std::cout << "[WeatherCloud][WARN] STAGE7_WEATHER_TEXTURE_FALLBACK"
...
void HwaSimIR::UpdateStage7SkyHorizon(const IRRuntimeEnvironment& environment, const char* reason, bool forceLog)
{
	...
	if (!m_skyNode.is_empty())
	{
		...
		SetShaderInputCached(m_skyNode, "u_game_world_haze", LVecBase2f(!m_p6.enabled&&m_cloudAppearance.enabled&&m_cloudAppearance.bounded?1.f:0.f,0));
	...
	if (!m_stage7LowerShellNode.is_empty())
	{
		...
		SetShaderInputCached(m_stage7LowerShellNode, "u_game_world_haze", LVecBase2f(!m_p6.enabled&&m_cloudAppearance.enabled&&m_cloudAppearance.bounded?1.f:0.f,0));
...
void HwaSimIR::InitCommonCaptureTask() {
	...
	std::cout << "[FrameCaptureScheduling] mode=post_do_frame_main_thread"
		<< " stalePreRenderCapture=disabled" << std::endl;

}

void HwaSimIR::InitHeadlessSceneCamera(int width, int height)
{
	if (!IsHeadlessOffscreenMode())
	{
		return;
	}
	if (m_renderRoot.is_empty())
	{
		m_renderRoot = NodePath("HeadlessOffscreenRenderRoot");
	}
	if (!m_headlessCameraNode.is_empty())
	{
		m_cameraNode = m_headlessCameraNode;
		if (m_cameraLens != nullptr)
		{
	...
	PT(Camera) headlessCamera = new Camera("HeadlessSensorCamera");
	PT(PerspectiveLens) lens = new PerspectiveLens();
	lens->set_fov(0.1, 0.1);
	lens->set_near_far(1.0, 100000.0);
	headlessCamera->set_lens(lens);
	m_headlessCameraNode = m_renderRoot.attach_new_node(headlessCamera);
	m_headlessCameraNode.set_pos(0.0f, 0.0f, 0.0f);
	m_headlessCameraNode.set_hpr(0.0f, 0.0f, 0.0f);
	m_cameraNode = m_headlessCameraNode;
	m_camera = headlessCamera;
	m_cameraLens = lens;

	std::cout << "[RenderBackend]"
		<< " mode=" << m_renderPresentationModeName
		<< " headlessCamera=HeadlessSensorCamera"
		<< " size=" << std::max(1, width) << "x" << std::max(1, height)
		<< " fov=0.1,0.1"
		<< " nearFar=1,100000"
		<< std::endl;
}
...
void HwaSimIR::InitPlatformModels()
{
	...
	if (IsHeadlessOffscreenMode())
	{
		InitHeadlessSceneCamera(m_headlessWidth, m_headlessHeight);
		std::cout << "[RenderBackend]"
			<< " mode=" << m_renderPresentationModeName
			<< " platformModelPaths=" << m_platformResMap.size()
			<< " modelRenderHost=headless_render_root"
			<< std::endl;
		return;
	}
...
{
	std::cout << "[M1 ResourceP0] platformType=" << type
		<< " model=" << AbsolutePathForLog(res.modelPath)
		<< " baseTexture=" << AbsolutePathForLog(res.texturePath)
		<< " materialIdTif=" << AbsolutePathForLog(res.materialIdTexturePath)
		<< " materialXml=" << AbsolutePathForLog(res.materialMapPath)
		<< " materialIdExists=" << (FileExists(res.materialIdTexturePath) ? 1 : 0)
		<< " materialXmlExists=" << (FileExists(res.materialMapPath) ? 1 : 0)
		<< std::endl;
	if (m_renderRoot.is_empty())
	{
		std::cout << "[RenderBackend][WARN]"
			...
			<< " failure=render_root_unavailable"
			<< std::endl;
		return NodePath();
	}
	...
	{
		modelNode = m_renderRoot.attach_new_node(loadedNode);
	}
	...
	if (modelNode.calc_tight_bounds(modelBoundsMin, modelBoundsMax))
	{
		std::cout << "[PlatformModelBounds]"
			...
			<< " coordinateFrame=panda_model_local"
			<< " min=(" << modelBoundsMin.get_x() << "," << modelBoundsMin.get_y() << "," << modelBoundsMin.get_z() << ")"
			<< " max=(" << modelBoundsMax.get_x() << "," << modelBoundsMax.get_y() << "," << modelBoundsMax.get_z() << ")"
			<< std::endl;
	}
		...
		if (it->type == PendingNetworkCommandType::Control)
		{
				...
				(m_bSyncRenderMode.load() || m_asyncInputPolicy == "OrderedQueue"))
			{
					...
					nowNs - lastIngressNs < quietRequiredNs;
				{
					...
					if (hasPendingRealtime || ingressMayStillBeStaging)
					{
				...
				if (hasPendingRealtime || ingressMayStillBeStaging) break;
			}
...
void HwaSimIR::ProcessControlCmdOnMainThread(const BYHWICD::ControlP2cX1ObjTrackingCmd& cmd) {
	...
	case 3: // 停止
		...
		std::cout << "[SyncRoundConservation]"
			...
			<< " lastCapturedSourceSeq=" << m_lastCapturedSourceSeq
			<< " inputMinusCaptured=" << (m_udpSequence >= m_lastCapturedSourceSeq
				? m_udpSequence - m_lastCapturedSourceSeq : 0)
...
void HwaSimIR::ProcessInitCmdOnMainThread(const BYHWICD::InitP2cObjectTrackingCmd& cmd,
	...
	std::cout << "[Stage0] Init baseline: sensorBand=" << sensor.trackerSensorBand
		...
		<< ", illuminatorAngleMrad=" << sensor.illuminatorAngle
		<< ", illuminatorSpotRadRaw=" << sensor.illuminatorSpotRad
...
{
	const std::string configMaterialPath = FirstExistingPath({
		"Config/Materials/MaterialDatabase.csv",
		"../Bin/Config/Materials/MaterialDatabase.csv",
		"HwaSim_IR/Bin/Config/Materials/MaterialDatabase.csv",
		"../HwaSim_IR/Bin/Config/Materials/MaterialDatabase.csv"
	});
	const std::string rootMaterialPath = FirstExistingPath({
		"materials/MaterialDatabase.csv",
		"../materials/MaterialDatabase.csv",
		"../../materials/MaterialDatabase.csv"
	});
	std::string materialPath = FirstExistingPath({configMaterialPath, rootMaterialPath});
	const std::string configBandOpticsPath = FirstExistingPath({
		"Config/Materials/MaterialBandOptics.csv",
		"../Bin/Config/Materials/MaterialBandOptics.csv",
		"HwaSim_IR/Bin/Config/Materials/MaterialBandOptics.csv",
		"../HwaSim_IR/Bin/Config/Materials/MaterialBandOptics.csv"
	});
	const std::string rootBandOpticsPath = FirstExistingPath({
		"materials/MaterialBandOptics.csv",
		"../materials/MaterialBandOptics.csv",
		"../../materials/MaterialBandOptics.csv"
	});
	std::string materialBandOpticsPath = FirstExistingPath({configBandOpticsPath, rootBandOpticsPath});
	...
	std::string modtranBandLutPath = FirstExistingPath({
		"Config/Atmosphere/MODTRAN/processed/band_lut_si.csv",
		"../Bin/Config/Atmosphere/MODTRAN/processed/band_lut_si.csv",
	...
	m_l2ActiveIlluminatorConfig.enabled = m_runtimeConfig.getBool("ActiveIlluminator", "Enable", "ActiveIlluminatorEnable", false, nullptr);
	m_l2ActiveIlluminatorConfig.band = IRActiveIlluminator::parseBand(
		m_runtimeConfig.getString("ActiveIlluminator", "Band", "ActiveIlluminatorBand", "NIR", nullptr));
	m_l2ActiveIlluminatorConfig.centerWavelengthUm = std::max(0.1,
		m_runtimeConfig.getDouble("ActiveIlluminator", "CenterWavelengthUm", "ActiveIlluminatorCenterWavelengthUm", 0.85, nullptr));
	m_l2ActiveIlluminatorConfig.bandwidthUm = std::max(0.0,
		m_runtimeConfig.getDouble("ActiveIlluminator", "BandwidthUm", "ActiveIlluminatorBandwidthUm", 0.05, nullptr));
	m_l2AngleInterpretation = m_runtimeConfig.getString("ActiveIlluminator", "AngleInterpretation", "ActiveIlluminatorAngleInterpretation", "FullCone", nullptr);
	m_l2ActiveIlluminatorConfig.beamProfile = IRActiveIlluminator::parseBeamProfile(
		m_runtimeConfig.getString("ActiveIlluminator", "BeamProfile", "ActiveIlluminatorBeamProfile", "Gaussian", nullptr));
	m_l2ActiveIlluminatorConfig.intensityMode = IRActiveIlluminator::parseIntensityMode(
		m_runtimeConfig.getString("ActiveIlluminator", "IntensityMode", "ActiveIlluminatorIntensityMode", "LegacyNormalized", nullptr));
	m_l2ActiveIlluminatorConfig.referenceRangeM = std::max(1.0,
		m_runtimeConfig.getDouble("ActiveIlluminator", "ReferenceRangeM", "ActiveIlluminatorReferenceRangeM", 1000.0, nullptr));
	m_l2ActiveIlluminatorConfig.referenceIrradianceWm2 = std::max(0.0,
		m_runtimeConfig.getDouble("ActiveIlluminator", "ReferenceIrradianceWm2", "ActiveIlluminatorReferenceIrradianceWm2", 1.0, nullptr));
	m_l2ActiveIlluminatorConfig.legacyMaxReferenceIrradianceWm2 = std::max(0.0,
		m_runtimeConfig.getDouble("ActiveIlluminator", "LegacyMaxReferenceIrradianceWm2", "ActiveIlluminatorLegacyMaxReferenceIrradianceWm2", 1.0, nullptr));
	m_l2ActiveIlluminatorConfig.useAtmosphericAttenuation = m_runtimeConfig.getBool(
		"ActiveIlluminator", "UseAtmosphericAttenuation", "ActiveIlluminatorUseAtmosphericAttenuation", true, nullptr);
	m_l2ActiveIlluminatorConfig.useGeometricOcclusion = m_runtimeConfig.getBool(
		"ActiveIlluminator", "UseGeometricOcclusion", "ActiveIlluminatorUseGeometricOcclusion", true, nullptr);
	m_l2ShadowUpdateHz = ClampStage5Double(m_runtimeConfig.getDouble(
		"ActiveIlluminator", "ShadowUpdateHz", "ActiveIlluminatorShadowUpdateHz", 30.0, nullptr), 1.0, 30.0);
	m_l2ActiveIlluminatorConfig.debugLog = m_runtimeConfig.getBool(
		"ActiveIlluminator", "DebugLog", "ActiveIlluminatorDebugLog", false, nullptr);
	m_l2ActiveIlluminatorConfig.debugVisualCone = m_runtimeConfig.getBool(
		"ActiveIlluminator", "DebugVisualCone", "ActiveIlluminatorDebugVisualCone", false, nullptr);
	if (m_l2AngleInterpretation != "FullCone")
	{
		std::cout << "[L2 ActiveIlluminator][WARN] AngleInterpretation=" << m_l2AngleInterpretation
			<< " fallback=FullCone reason=only_audited_protocol_interpretation" << std::endl;
		m_l2AngleInterpretation = "FullCone";
	}
	...
	if (m_m1HumidityProfile != "default")
	{
		std::cout << "[M1 Physics][WARN] humidity_profile=" << m_m1HumidityProfile
			<< " fallback=default reason=only_audited_profile_available" << std::endl;
	...
	WarnIfMaterialCopiesDrift("MaterialDatabase.csv", configMaterialPath, rootMaterialPath);
	WarnIfMaterialCopiesDrift("MaterialBandOptics.csv", configBandOpticsPath, rootBandOpticsPath);
	std::cout << "[L2 ActiveIlluminatorConfig] Enable=" << (m_l2ActiveIlluminatorConfig.enabled ? 1 : 0)
		<< " Band=" << IRActiveIlluminator::bandName(m_l2ActiveIlluminatorConfig.band)
		<< " CenterWavelengthUm=" << m_l2ActiveIlluminatorConfig.centerWavelengthUm
		<< " BandwidthUm=" << m_l2ActiveIlluminatorConfig.bandwidthUm
		<< " AngleInterpretation=" << m_l2AngleInterpretation
		<< " BeamProfile=" << IRActiveIlluminator::beamProfileName(m_l2ActiveIlluminatorConfig.beamProfile)
		<< " IntensityMode=" << IRActiveIlluminator::intensityModeName(m_l2ActiveIlluminatorConfig.intensityMode)
		<< " ReferenceRangeM=" << m_l2ActiveIlluminatorConfig.referenceRangeM
		<< " ReferenceIrradianceWm2=" << m_l2ActiveIlluminatorConfig.referenceIrradianceWm2
		<< " LegacyMaxReferenceIrradianceWm2=" << m_l2ActiveIlluminatorConfig.legacyMaxReferenceIrradianceWm2
		<< " UseAtmosphericAttenuation=" << (m_l2ActiveIlluminatorConfig.useAtmosphericAttenuation ? 1 : 0)
		<< " UseGeometricOcclusion=" << (m_l2ActiveIlluminatorConfig.useGeometricOcclusion ? 1 : 0)
		<< " DebugVisualCone=" << (m_l2ActiveIlluminatorConfig.debugVisualCone ? 1 : 0)
		<< " productionGate=" << (m_l2ActiveIlluminatorConfig.enabled ? "requested" : "controlled_off")
		<< std::endl;
	...
	m_m1NirDisplayRadianceMin = std::max(0.0, m_runtimeConfig.getDouble(
	...
	m_m1MwirSkyEmissivity = ClampStage5Double(m_runtimeConfig.getDouble(
		"M1RadianceDisplay", "MWIRSkyEmissivity", "M1MWIRSkyEmissivity", 0.98, nullptr), 0.0, 1.0);
	m_m1MwirGroundEffectiveTempK = ClampStage5Double(m_runtimeConfig.getDouble(
		"M1RadianceDisplay", "MWIRGroundEffectiveTempK", "M1MWIRGroundEffectiveTempK", 288.0, nullptr), 180.0, 360.0);
	m_m1MwirGroundEmissivity = ClampStage5Double(m_runtimeConfig.getDouble(
	...
	{
		m_m1NirDisplayRadianceMax = m_m1NirDisplayRadianceMin + 350.0;
		std::cerr << "[M1 RadianceDisplay][WARN] band=NIR fallbackReason=invalid_public_range" << std::endl;
	}
	if (!std::isfinite(m_m1MwirDisplayRadianceMax) ||
		m_m1MwirDisplayRadianceMax <= m_m1MwirDisplayRadianceMin + 1.0e-9)
	{
	...
	std::cout << "[M1 RadianceDisplay] mode=scene_wide_physical_window"
		...
		<< " MWIRGroundTempK=" << m_m1MwirGroundEffectiveTempK
		<< " environmentSource=config_engineering_proxy"
		<< " perTargetGain=forbidden responseMode=RectangularBand" << std::endl;
	{
	...
	{
		IRModtranRadianceQuery goldenQuery;
		...
		if (!goldenPass)
		{
			m_stage5ModtranRadianceReady = false;
		}
	}
	m_stage5ModtranRadianceCache.clear();
	m_lastStage5ModtranPathABLogState.clear();
    ...
    try{
        ...
        if(!m_p6.enabled){
            ...
            g.streamingRadiusM=m_stage7VolumeConfig.streamingRadiusM;g.deactivationRadiusM=m_stage7VolumeConfig.deactivationRadiusM;
            g.maxActiveVolumes=m_stage7VolumeConfig.maxActiveVolumes;g.maxVisibleVolumes=m_stage7VolumeConfig.maxVisibleVolumes;
            g.fadeDistanceM=m_stage7VolumeConfig.fadeDistanceM;
            m_stage7VolumeStreaming.setConfig(g);m_stage7VolumeConfig=m_stage7VolumeStreaming.config();
            if(m_cloudAppearance.enabled){
                m_stage7VolumeDensityTemplateCount=int(m_cloudAppearance.templates.size());
                m_stage7VolumeDensityTextureSize=m_cloudAppearance.size;
                m_stage7VolumeRaymarchStepsNear=m_cloudAppearance.nearSteps;
                m_stage7VolumeRaymarchStepsMedium=m_cloudAppearance.mediumSteps;
                m_stage7VolumeRaymarchStepsFar=m_cloudAppearance.farSteps;
                m_stage7CloudWorldSizeM=m_cloudAppearance.sheetCoverage;m_stage7CloudTextureWorldSizeM=m_cloudAppearance.sheetPeriod;
                m_stage7CloudBaseAltitudeM=m_cloudAppearance.sheetAltitude;
            }
            std::cout<<"[CloudWorldIdentity] revision="<<m_cloudAppearance.revision<<" configHash="<<m_cloudAppearance.hash
                <<" seed="<<g.weatherSeed<<" cellSizeM="<<g.cellSizeM<<" appearance="<<appearance
                <<" frame=WGS84_ENU_tangent static=1 positionSource=IRWorldCloudStreaming templateSelection=stable_descriptor"
                <<" maxVisible="<<g.maxVisibleVolumes<<" maxActive="<<g.maxActiveVolumes<<std::endl;
        }
    }catch(const std::exception& e){std::cerr<<"[CloudWorldIdentity][FATAL] "<<e.what()<<std::endl;std::exit(2);}
	...
	std::cout << "[M1 ResourceP0]"
		...
		<< " ModtranRows=" << m_stage5ModtranRadianceLut.entryCount()
		<< " default_NVG=" << sensorWaveResolvedDir << "/default_NVG.json"
		<< " default_MWIR=" << sensorWaveResolvedDir << "/default_MWIR.json"
		<< " SensorWaveLoaded=" << (m_irSensorProfilesReady ? 1 : 0)
		<< std::endl;
...
{
	if ((IsVisibleWindowMode() && !m_pMainWindow) || m_cameraNode.is_empty())
	{
		std::cout << "[RenderBackend][WARN]"
			<< " mode=" << m_renderPresentationModeName
			<< " skyAndCloudScene=skipped"
			<< " cameraReady=" << (m_cameraNode.is_empty() ? "0" : "1")
			<< " visibleWindowReady=" << (m_pMainWindow != nullptr ? "1" : "0")
			<< std::endl;
		return;
	}
...
void HwaSimIR::InitInfraredShader() {
...
#endif
    varying highp vec2 v_cloud_world_uv;
...
#endif
        v_cloud_world_uv = (u_cloud_render_to_world*world_pos).xy * u_cloud_world_uv_reciprocal.x;
        v_cloud_eye=(p3d_ModelViewMatrix*p3d_Vertex).xyz;
    }
    )";
	...
	uniform int u_m1_physics_runtime_en;
	uniform float u_m1_tau_up;
	uniform float u_m1_path_radiance;
	uniform float u_m1_direct_solar_irradiance;
	uniform float u_m1_sky_diffuse_irradiance;
	...
	uniform float u_l2_active_tau_outbound;
...
#if P1_M1_L1_L2_SHADER_FEATURES
    highp float P4PlanckWm2SrUm(highp float wavelength_um, highp float temperature_K)
    {
		highp float safe_lambda = clamp(wavelength_um, 0.1, 100.0);
		highp float safe_temp = clamp(temperature_K, 1.0, 6000.0);
		highp float exponent_value = clamp(14387.752 / (safe_lambda * safe_temp), 0.000001, 80.0);
		return 119104200.0 / (pow(safe_lambda, 5.0) * max(exp(exponent_value) - 1.0, 0.000001));
    }

    highp float P4MwirBandMeanPlanckWm2SrUm(highp float temperature_K)
    {
		// Rectangular 3--5 um response, composite Simpson integration with
		// 0.5 um samples.  The /12 includes both Simpson's h/3 and division
		// by the 2 um bandwidth, matching IRRadianceModelV2 exactly.
		return (P4PlanckWm2SrUm(3.0, temperature_K)
			+ 4.0 * P4PlanckWm2SrUm(3.5, temperature_K)
			+ 2.0 * P4PlanckWm2SrUm(4.0, temperature_K)
    ...
    void main() {
        ...
        if (u_object_kind == 1 || u_is_background == 1) {
            if (u_stage7_sky_horizon_en == 1) {
                ...
                if(u_game_world_haze.x>0.5){
                    // Both hemispheres agree at the real world horizon. This
                    // directional transition follows the camera pitch naturally.
                    float elevation=normalize(v_local_pos).z;
        ...
        if (u_object_kind == 2) {
            ...
            float localFade=0.0;
            highp vec2 cloudWorldXY=v_cloud_world_uv/max(u_cloud_world_uv_reciprocal.x,0.000001);
            for(int i=0;i<4;++i) {
            ...
            if(u_game_sheet.w>0.0){
                // A distant horizontal sheet becomes sub-pixel at grazing
                // angles. Fade that coverage continuously instead of a hard line.
                highp vec3 planeNormal=normalize(mat3(p3d_ModelViewMatrix)*vec3(0,0,1));
                highp float grazing=abs(dot(normalize(v_cloud_eye),planeNormal));
                cloud_alpha*=smoothstep(0.0,u_game_sheet.w,grazing);
            }
	...
	fragment_shader += R"(
        if (u_stage5_radiance_debug_en == 1) {
			...
			if (u_l2_active_en == 1) {
				...
				if (beam_angle_rad <= u_l2_active_half_angle_rad && u_l2_active_half_angle_rad > 0.0) {
					if (u_l2_active_beam_profile == 1) {
						...
						float normalized_angle = beam_angle_rad / u_l2_active_half_angle_rad;
						beam_factor = exp(-2.77258872 * normalized_angle * normalized_angle);
					}
				}
            ...
            if (u_stage5_debug_view_mode == 0 && u_m1_physics_runtime_en == 1 && u_ir_band_index == 1) {
				...
				float rear_coverage = clamp(stage5_rear_mask, 0.0, 1.0);
			...
			// The formal M1 branches already include MODTRAN tau and path radiance.
			// Applying the empirical Stage7 fog/contrast transform here would
			// attenuate the target a second time.  Cloud/precipitation geometry is
			// still composited independently after the opaque scene.
...
void HwaSimIR::ApplyInfraredShader(NodePath& node, bool isBackground) {
	...
	SetShaderInputCached(node, "u_m1_sky_diffuse_irradiance", LVecBase2f(0.0f, 0.0f));
	SetShaderInputCached(node, "u_m1_sun_visibility", LVecBase2f(1.0f, 0.0f));
	SetShaderInputCached(node, "u_m1_sky_visibility", LVecBase2f(1.0f, 0.0f));
	SetShaderInputCached(node, "u_m1_sun_direction_world", LVecBase3f(0.0f, 0.0f, 1.0f));
	SetShaderInputCached(node, "u_m1_display_scale", LVecBase2f(1.0f, 0.0f));
	SetShaderInputCached(node, "u_m1_display_offset", LVecBase2f(0.0f, 0.0f));
	SetShaderInputCached(node, "u_m1_display_gamma", LVecBase2f(1.0f, 0.0f));
	SetShaderInputCached(node, "u_l2_active_en", LVecBase2i(0, 0));
	...
	SetShaderInputCached(node, "u_l2_active_tau_outbound", LVecBase2f(1.0f, 0.0f));
    ...
    node.set_shader_input("u_cloud_render_to_world",LMatrix4f::ident_mat());
...
IRModtranRadianceResult HwaSimIR::QueryStage5ModtranRadiance(const TargetPlatformData& targetPlat, const IRRuntimeEnvironment& environment, const IRObjectRadianceOutput& radiance, const std::string& targetKey)
{
	...
	std::ostringstream key;
	key << std::fixed << std::setprecision(9);
	key << targetKey
...
{
	return m_m1CompareOnly || m_m1RuntimeEnabled || m_l2ActiveIlluminatorConfig.enabled ||
		m_enableStage5ModtranRadianceDebug ||
...
void HwaSimIR::LogStage5ModtranRadianceCompare(const TargetPlatformData& targetPlat, const IRRadianceComponents& components, const IRModtranRadianceResult& modtranResult, double rangeKm, double observerAltKm, double targetAltKm)
{
	...
	std::cout << "[Stage5 ModtranRadianceCompare]"
		...
		<< " legacyTauUp=" << components.tauUp
		<< " m1TauUp=" << components.m1TauUp
		<< " directSolar=" << components.directSolarIrradiance
		<< " skyDiffuse=" << components.skyDiffuseIrradiance
		<< " pathScattering=" << components.pathScatteringRadiance
		<< " pathThermal=" << components.pathThermalRadiance
		<< " solarReflected=" << components.solarReflectedRadiance
		<< " skyReflected=" << components.skyReflectedRadiance
		<< " m1Surface=" << components.m1SurfaceRadiance
		<< " m1LocalSurfacePeak=" << (components.rearHotspotRadiance + components.brightspotRadiance)
		<< " m1LocalSensorPeak=" << (components.m1TauUp *
			(components.rearHotspotRadiance + components.brightspotRadiance))
		...
		<< " fallbackQuery=" << modtranResult.fallbackQuery
		...
		<< " ratio=" << ratio
		<< " diff=" << diff
		<< " unitRadiance=" << modtranResult.radianceUnit
		<< " unitIrradiance=" << modtranResult.irradianceUnit
		<< " responseMode=" << modtranResult.responseMode
		<< " sourceFiles=" << modtranResult.sourceFiles
		<< std::endl;
...
void HwaSimIR::ApplyStage5RadianceDebug(TargetPlatformData& targetPlat, const IRObjectRadianceOutput& radiance, const IRHotspotState& rearHotspot, const IRBrightSpotState& brightSpot, bool rearEnabledForShader, float rearIntensityForShader, const std::string& targetKey, float dtSec)
{
	...
	else
	{
		...
		modtranRadiance.sourceFiles = "disabled";
	}
	...
	if (l2ProtocolRequested && !l2Atmosphere.valid && l2Atmosphere.fallbackAxis == "solarZenithDeg")
	{
		...
		// audited 45-degree solar grid slice, with an explicit query-mode label.
		IRRuntimeEnvironment activeTauEnvironment = environment;
		activeTauEnvironment.sunElevationDeg = 45.0;
		l2Atmosphere = QueryStage5ModtranRadiance(
			targetPlat, activeTauEnvironment, radiance, targetKey + ":l2_active_tau");
		if (l2Atmosphere.valid)
		{
	...
	stage5Input.activeSensorRadiance = l2Active.activeSensorRadianceWm2SrUm;
	stage5Input.activeContributionEnabled = l2Active.activeContributionEnabled;
	stage5Input.activeFallbackReason = l2Active.fallbackReason;
	const bool l2ShaderActiveEnabled =
		stage5Input.m1RuntimeAffectsImage &&
		m_l2ActiveIlluminatorConfig.enabled &&
		m_realTimeSceneData.weaponState.illuminatorEn &&
		l2Active.spectralOverlap &&
		l2Active.halfAngleRad > 0.0 &&
		l2Active.referenceIrradianceWm2 > 0.0 &&
		l2Active.spectralOverlapWidthUm > 0.0 &&
		(!m_l2ActiveIlluminatorConfig.useAtmosphericAttenuation || l2Atmosphere.valid);
	...
	if (l2Active.activeContributionEnabled)
	{
		...
		(a1TraceStateChanged || frameSeq <= 3 || (frameSeq % (m_quietPerfMode ? 3600 : 120)) == 0))
	{
		...
		std::cout << "[A1 ActiveDisplayTrace]"
			...
			<< " localSensorPeak=" << (components.m1TauUp *
				(components.rearHotspotRadiance + components.brightspotRadiance))
	...
	if (modtranCompareEnabled && stage5LogDue)
	{
		std::cout << "[M1 Compare]"
			...
			<< " targetKey=" << targetKey
			<< " band=" << IRBandName(components.band)
			<< " observerAltKm=" << observerAltKmForLog
			<< " targetAltKm=" << targetAltKmForLog
			<< " rangeKm=" << rangeKmForLog
			<< " visibilityKm=" << (environment.visibilityMeters / 1000.0)
			<< " solarZenithDeg=" << (90.0 - environment.sunElevationDeg)
			<< " tauUp=" << components.m1TauUp
			<< " directSolar=" << components.directSolarIrradiance
			<< " skyDiffuse=" << components.skyDiffuseIrradiance
			<< " pathScattering=" << components.pathScatteringRadiance
			<< " pathThermal=" << components.pathThermalRadiance
			<< " rhoNir=" << components.reflectance
			<< " reflectanceSource=" << stage5Input.reflectanceSource
			<< " solarReflected=" << components.solarReflectedRadiance
			<< " skyReflected=" << components.skyReflectedRadiance
			<< " activeSurface=" << components.activeSurfaceRadiance
			<< " activeSensor=" << components.activeSensorRadiance
			<< " activeEnabled=" << (components.activeContributionEnabled ? 1 : 0)
			<< " activeFallbackReason=" << components.activeFallbackReason
			<< " m1Surface=" << components.m1SurfaceRadiance
			<< " m1LocalSurfacePeak=" << (components.rearHotspotRadiance + components.brightspotRadiance)
			<< " m1LocalSensorPeak=" << (components.m1TauUp *
				(components.rearHotspotRadiance + components.brightspotRadiance))
			<< " m1Sensor=" << components.m1SensorRadiance
			<< " legacySensor=" << components.sensorInputLegacy
			<< " finalOutput=" << (stage5Input.m1RuntimeAffectsImage ? "M1" : "legacy")
			<< " compareOnly=" << (m_m1CompareOnly ? 1 : 0)
			<< " interpolation=" << modtranRadiance.interpolationMode
			<< " valid=" << (modtranRadiance.valid ? 1 : 0)
			<< " fallbackReason=" << modtranRadiance.fallbackReason
			<< " fallbackAxis=" << (modtranRadiance.fallbackAxis.empty() ? "none" : modtranRadiance.fallbackAxis)
			<< " fallbackQuery=" << modtranRadiance.fallbackQuery
			<< " fallbackMin=" << modtranRadiance.fallbackMin
			<< " fallbackMax=" << modtranRadiance.fallbackMax
			<< " responseMode=" << modtranRadiance.responseMode
			<< std::endl;
	}
...
void HwaSimIR::UpdateM1SolarPosition(IRRuntimeEnvironment& environment, bool forceLog)
{
	IRSolarPositionInput input;
	std::string positionSource = "weather_profile";
	const IRWeatherMetadata& weather = m_irWeatherProfile.metadata();
	input.latitudeDeg = weather.latitudeDeg;
	input.longitudeDeg = weather.longitudeDeg;
	input.altitudeM = 0.0;
	if (m_stage0DisplayFrameCount > 0 &&
		std::isfinite(m_realTimeSceneData.platLoc.lat) && std::isfinite(m_realTimeSceneData.platLoc.lon) &&
		m_realTimeSceneData.platLoc.lat >= -90.0 && m_realTimeSceneData.platLoc.lat <= 90.0 &&
		m_realTimeSceneData.platLoc.lon >= -180.0 && m_realTimeSceneData.platLoc.lon <= 180.0)
	{
		input.latitudeDeg = m_realTimeSceneData.platLoc.lat;
		input.longitudeDeg = m_realTimeSceneData.platLoc.lon;
		input.altitudeM = IsReasonableAltitudeMeters(m_realTimeSceneData.platLoc.alt)
			? m_realTimeSceneData.platLoc.alt : 0.0;
		positionSource = "protocol_realtime_platform";
	}
	else if (m_isAddPlatform &&
		std::isfinite(m_initSceneData.platParamInit.spatial.lat) && std::isfinite(m_initSceneData.platParamInit.spatial.lon) &&
		m_initSceneData.platParamInit.spatial.lat >= -90.0 && m_initSceneData.platParamInit.spatial.lat <= 90.0 &&
		m_initSceneData.platParamInit.spatial.lon >= -180.0 && m_initSceneData.platParamInit.spatial.lon <= 180.0)
	{
		input.latitudeDeg = m_initSceneData.platParamInit.spatial.lat;
		input.longitudeDeg = m_initSceneData.platParamInit.spatial.lon;
		input.altitudeM = IsReasonableAltitudeMeters(m_initSceneData.platParamInit.spatial.alt)
			? m_initSceneData.platParamInit.spatial.alt : 0.0;
		positionSource = "protocol_init_platform";
	}
	if (!IRSolarPosition::parseUtcDate(m_m1FallbackUtcDate, input.utcYear, input.utcMonth, input.utcDay))
	{
		input.utcYear = 2026;
		input.utcMonth = 9;
		input.utcDay = 6;
		std::cout << "[M1 SolarPosition][WARN] FallbackUtcDate=" << m_m1FallbackUtcDate
			<< " fallback=2026-09-06 reason=invalid_yyyy_mm_dd" << std::endl;
	}
	...
	m_lastM1SolarLogState = state.str();
	std::cout << "[M1 SolarPosition]"
		<< " lat=" << input.latitudeDeg << " lon=" << input.longitudeDeg << " altM=" << input.altitudeM
		<< " UTC=" << input.utcYear << '-' << std::setw(2) << std::setfill('0') << input.utcMonth
		<< '-' << std::setw(2) << input.utcDay << 'T' << std::fixed << std::setprecision(6) << input.utcHour << "h"
		<< std::setfill(' ') << " solarAz=" << m_m1SolarState.azimuthDeg
...
void HwaSimIR::UpdateL2ActiveVisibility(double currentTime)
{
	...
	m_l2ActiveVisibilityByTarget.clear();
	if (m_cameraNode.is_empty()) return;
	...
	const LPoint3f source = m_cameraNode.get_pos(m_renderRoot);
	for (size_t i = 0; i < volumes.size(); ++i)
	{
		double visibility = 1.0;
		const LVecBase3f sourceToTarget = volumes[i].center - source;
		const float targetRange = sourceToTarget.length();
		if (targetRange <= 1.0e-4f)
		{
			visibility = 0.0;
		}
		else if (m_l2ActiveIlluminatorConfig.useGeometricOcclusion)
		{
			LVecBase3f ray = sourceToTarget / targetRange;
			for (size_t j = 0; j < volumes.size(); ++j)
			{
				if (i == j) continue;
				const LVecBase3f sourceToOccluder = volumes[j].center - source;
				const float along = sourceToOccluder.dot(ray);
				if (along <= volumes[j].radius || along >= targetRange - volumes[i].radius) continue;
				const float perpendicular = (sourceToOccluder - ray * along).length();
				if (perpendicular < volumes[j].radius)
				{
					visibility = 0.0;
					break;
		...
		m_l2ActiveVisibilityByTarget[volumes[i].key] = visibility;
	}
}

double HwaSimIR::L2ActiveVisibilityForTarget(const std::string& targetKey) const
{
	if (!m_l2ActiveIlluminatorConfig.useGeometricOcclusion) return 1.0;
	const std::map<std::string, double>::const_iterator it = m_l2ActiveVisibilityByTarget.find(targetKey);
	return it != m_l2ActiveVisibilityByTarget.end() ? it->second : 0.0;
}

IRActiveIlluminatorOutput HwaSimIR::EvaluateL2ActiveIlluminator(
	...
	const IRModtranRadianceResult& modtranRadiance)
{
	...
	input.protocolAngleMrad = m_sensorParam.illuminatorAngle;
	input.protocolSpotRadiance = m_sensorParam.illuminatorSpotRad;
	input.tauInbound = modtranRadiance.tauUp;
	...
	if (!m_cameraNode.is_empty())
	{
		sourceDirection = m_renderRoot.get_relative_vector(m_cameraNode, LVecBase3f(0.0f, 1.0f, 0.0f));
	}
	if (sourceDirection.length_squared() > 1.0e-8f) sourceDirection.normalize();
	const LVecBase3f sourceToTarget = targetCenter - sourcePosition;
	input.rangeM = sourceToTarget.length();
	if (input.rangeM > 1.0e-6)
	{
	...
	std::ostringstream state;
	state << (input.protocolEnabled ? 1 : 0) << ':' << (output.activeContributionEnabled ? 1 : 0)
		<< ':' << output.sourceBand << ':' << output.sensorBand << ':' << output.fallbackReason
		<< ':' << std::fixed << std::setprecision(6) << output.protocolAngleMrad
		<< ':' << output.referenceIrradianceWm2 << ':' << output.activeSensorRadianceWm2SrUm;
	const bool stateChanged = m_l2LastActiveLogState[targetKey] != state.str();
		...
		(seq % (m_quietPerfMode ? 3600 : 120)) == 0)
	{
		std::cout << "[L2 ActiveIlluminator] sourceSeq=" << seq
			...
			<< " halfAngleRad=" << output.halfAngleRad
			<< " rangeM=" << output.rangeM
			<< " beamAngleRad=" << output.beamAngleRad
			<< " beamFactor=" << output.beamFactor
			<< " spotRadiusM=" << output.spotRadiusM
			<< " ErefWm2=" << output.referenceIrradianceWm2
			<< " EgeomWm2=" << output.geometricIrradianceWm2
			<< " tauOutbound=" << output.tauOutbound
			<< " tauInbound=" << output.tauInbound
			<< " EtargetWm2=" << output.targetIrradianceWm2
			...
			<< " tauFallbackAxis=" << (modtranRadiance.fallbackAxis.empty() ? "none" : modtranRadiance.fallbackAxis)
			<< " tauFallbackQuery=" << modtranRadiance.fallbackQuery
			<< " tauFallbackMin=" << modtranRadiance.fallbackMin
			<< " tauFallbackMax=" << modtranRadiance.fallbackMax
			<< " reflectanceSource=" << output.reflectanceSource
			<< " responseMode=RectangularBand"
...
void HwaSimIR::UpdatePlatformIRStatus() {
	...
	// One cache lifetime per ~30 Hz IR update: shared within the update, never
	// stale across moving geometry, and never grows with continuous positions.
	m_stage5ModtranRadianceCache.clear();
	...
	if (hasStage7Background)
	{
		...
		const double stage7IntervalSec = 1.0 / std::max(1.0, std::max(m_stage7UpdateHz, m_stage7CloudUpdateHz));
	...
	UpdateStage7CloudAnimationTime(current_time);
	if (profileBreakdown)
	{
...
void HwaSimIR::ResetRenderSchedulingState()
{
	...
	m_lastSourceSeqContinuous.store(true);
	m_syncFrameActive.store(false);
	m_frameRenderImageSeqBefore = 0;
	m_frameRenderImageSeqAfter = 0;
	m_frameRenderImageAdvanced = false;
	m_invalidRenderedFrameCount = 0;
	m_frameOutputHealthy = true;
	m_perfStats.reset();
	if (m_pTcpThread)
	{
		m_pTcpThread->resetFrameCounters();
...
AsyncTask::DoneStatus HwaSimIR::capture_task(GenericAsyncTask* task, void* data) {
	...
	if (!self->m_isSimRunning.load() || self->m_currentFrameTelemetry.sourceSeq == 0)
	{
		return AsyncTask::DS_cont;
	}
	...
	if (!self->m_frameRenderImageAdvanced)
	{
		++self->m_invalidRenderedFrameCount;
		if (self->m_invalidRenderedFrameCount <= 3 ||
			(self->m_invalidRenderedFrameCount % 120) == 0)
		{
			std::cerr << "[FrameValidity][ERROR] noBusinessVideoFrame=1"
				<< " reason=final_sensor_ram_image_not_advanced"
				<< " sourceSeq=" << self->m_currentFrameTelemetry.sourceSeq
				<< " imageSeqBefore=" << self->m_frameRenderImageSeqBefore
				<< " imageSeqAfter=" << self->m_frameRenderImageSeqAfter
				<< " renderPath=" << self->m_stage6RenderPath
				<< " invalidCount=" << self->m_invalidRenderedFrameCount
				<< std::endl;
		}
#if defined(HWASIMIR_HAS_ZRDDS)
		if (self->m_frameOutputHealthy)
		{
			self->m_frameOutputHealthy = false;
			self->PublishDdsVideoStatus(false, "invalid_rendered_frame");
		}
#endif
		return AsyncTask::DS_cont;
	}
	if (!self->m_frameOutputHealthy)
	{
		self->m_frameOutputHealthy = true;
#if defined(HWASIMIR_HAS_ZRDDS)
		self->PublishDdsVideoStatus(true, "rendered_frame_recovered");
#endif
		std::cout << "[FrameValidity] result=RECOVERED"
			<< " sourceSeq=" << self->m_currentFrameTelemetry.sourceSeq
			<< " imageSeq=" << self->m_frameRenderImageSeqAfter << std::endl;
	}
	...
	if (!self->m_firstValidVideoFrameLogged)
	{
		self->m_firstValidVideoFrameLogged = true;
		std::cout << std::fixed << std::setprecision(3)
			<< "[StartupTiming] milestone=first_valid_video_frame elapsedMs="
			<< (IRPerfStats::steadyTimeNs() - self->m_startupBeginTimeNs) / 1.0e6
			<< " sourceSeq=" << telemetry.sourceSeq
			<< " width=" << frameWidth
			<< " height=" << frameHeight
			<< std::endl;
