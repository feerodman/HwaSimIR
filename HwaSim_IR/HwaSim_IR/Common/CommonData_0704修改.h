...
namespace BYHWICD {
    ...
    struct trackerSensorParam {
        ...
        //跟瞄器安装参数，位置（米）
		double trackerX;  //todo，添加到相机初始化位置姿态
		double trackerY;
		double trackerZ;
		//跟瞄器安装参数，角度（°）
		double trackerPitch;
		...
		//照明器安装参数，位置（米）
		double illuminatorX;  //todo，暂不考虑，保留
		double illuminatorY;
		double illuminatorZ;
		//照明器安装参数，角度（°）
		double illuminatorPitch;  //todo，暂不考虑，保留
    ...
    struct InitObjectTrackingParam {
        bool enable;//是否挂载  //todo 配合初始化ID
        int envTerrain; //地形 0-戈壁 1-山区 2-海面
        int envSky; //天气 0-晴 1-云 2-雨 3-雪 4-雾 5-阴
        double envMaxHeightRain; //最大降雨高度（m）
        double envTransHeightRain; //降雨过渡高度（m）  //todo ,不需要的话备注
        double envMaxHeightSnow; //最大降雪高度（m）
        double envTransHeightSnow; //降雪过渡高度（m）
        double envRainSnowSpeedScale; //雨雪相对速度系数
        double envRadScaleTerrain;  //地形亮度调整系数  //todo ,改地面亮度
        double envRadScaleSky;  //天空亮度调整系数		//todo ,改天空亮度
        double envTemp; //温度（°）//todo ,给范围
        double envHumidity; //湿度（%）			//todo ,给范围
        double envVisibility; //能见度（米）//todo ,给范围
        double envWindV; //风速（m/s）//todo ,给范围
        double envWindDir; //风向（°），北偏东为正//todo ,给范围
		int simMode; //01-同步   02-异步，用videoFps
        int videoFps; //帧率，0为放开渲染帧率，其他值为锁帧
        trackerSensorParam trackerSensor[1]; //考虑是1/2进程
    };
    //回合控制信号
	...
	struct WeaponState {
		...
		int damageFlag; //目标是否毁伤 0-未毁 1-毁伤   //todo 保留不使用
